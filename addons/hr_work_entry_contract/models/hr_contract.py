...
class HrContract(models.Model):
    ...
    def _postprocess_attendance_intervals(self, intervals):
        ...
        # undesirable merges
        try:
            multi_type_intervals = [
                interval
                for interval in intervals
                if len(interval[2].work_entry_type_id) > 1
            ]
        ...
        for interval in multi_type_intervals:
            ...
            for attendance in attendances:
                if attendance.work_entry_type_id == current_work_entry_type:
                    end_date = interval[0].replace(
                        hour=floor(attendance.hour_to),
                    ...
                    start_date = interval[0].replace(
                        hour=floor(attendance.hour_from),
        ...
        for contract in self:
            ...
            for leave in itertools.chain(
                ...
                for resource in resources_list:
                    ...
                    leave_interval = contract._get_valid_leave_intervals(
            ...
            real_attendances = self._postprocess_attendance_intervals(
                attendances - leaves
            )
        ...
        if isinstance(date_start, datetime):
            ...
            for contract_tz, contracts in contracts_by_tz.items():
                ...
                contract_vals += contracts._get_contract_work_entries_values(
                    tz.localize(date_start), tz.localize(date_stop)
                )
            ...
            contracts_by_company_tz[
                (
                    ...
                    (
                        ...
                        or contract.employee_id.resource_calendar_id
                    ).tz,
                )
            ] += contract
    ...
    def _generate_work_entries(self, date_start, date_stop, force=False):
        ...
        for contract in self:
            ...
            tz = pytz.timezone(contract_tz) if contract_tz else pytz.utc
            contract_start = (
                tz.localize(fields.Datetime.to_datetime(contract.date_start))
                .astimezone(utc)
                .replace(tzinfo=None)
            )
            ...
            if contract.date_end:
                contract_stop = (
                    tz.localize(contract_stop).astimezone(utc).replace(tzinfo=None)
                )
    ...
    def write(self, vals):
        ...
        if not salary_simulation and any(
            key in dependendant_fields for key in vals.keys()
        ):
            for contract in self:
                date_from = max(
    ...
    def _cron_generate_missing_work_entries(self):
        ...
        contracts_todo = all_contracts.filtered(
