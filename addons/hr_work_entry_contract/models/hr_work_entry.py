    ...
    def _get_duration_batch(self):
        ...
        for (date_start, date_stop), employees_by_calendar in mapped_periods.items():
            for calendar, employees in employees_by_calendar.items():
                mapped_contract_data[(date_start, date_stop)][calendar] = (
        ...
        for work_entry in self - super_work_entries:
            ...
            calendar = work_entry.contract_id.resource_calendar_id
            employee = work_entry.contract_id.employee_id
            result[work_entry.id] = (
                mapped_contract_data[(date_start, date_stop)][calendar][employee.id][
                    "hours"
                ]
                if calendar
                else 0.0
        ...
        if (
            ...
            if not contracts:
                raise ValidationError(
                    _(
                        "%s does not have a contract from %s to %s.",
                        employee.name,
                        contract_start,
                        contract_end,
                    )
