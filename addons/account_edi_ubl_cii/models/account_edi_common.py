        ...
        if invoice:
            ...
            if logs:
                body += Markup("<ul>%s</ul>") % Markup().join(
                    Markup("<li>%s</li>") % l for l in logs
                )
                ...
                tax = self.env["account.tax"].search(
                    [
                        *self.env["account.tax"]._check_company_domain(
                        ...
                        ),  # Journal type is ensured by _create_invoice_from_xml_tree to be either 'sale' or 'purchase'
                    ],
                    limit=1,
                )
                if tax:
    ...
    def _import_fill_invoice_line_taxes(
        ...
        for tax_node in tax_nodes:
            ...
            if hasattr(invoice_line, "_predict_specific_tax"):
                ...
                predicted_tax_id = invoice_line._predict_specific_tax(
                    "percent", amount, invoice_line.move_id.journal_id.type
                )
    ...
    def _check_xml_ecosio(self, invoice, xml_content, ecosio_formats):
        ...
        for item in response["Result"]:
            if item["artifactPath"]:
                report.append(
                    Markup(
                        "<li><font style='color:Blue;'><strong>%s</strong></font></li>"
                    )
                    % item["artifactPath"]
                )
            for detail in item["Item"]:
                if detail["errorLevel"] == "WARN":
                    ...
                    report.append(
                        Markup(
                            "<li><font style='color:Orange;'><strong>%s</strong></font></li>"
                        )
                        % detail["errorText"]
                    )
                    ...
                    report.append(
                        Markup(
                            "<li><font style='color:Tomato;'><strong>%s</strong></font></li>"
                        )
                        % detail["errorText"]
                    )
        ...
        if errors_cnt == 0:
            invoice.message_post(
                body=Markup(
                    "<font style='color:Green;'><strong>ECOSIO: All clear for format %s!</strong></font>"
                )
                % ecosio_format
            )
        else:
            invoice.message_post(
                body=Markup(
                    "<font style='color:Tomato;'><strong>ECOSIO ERRORS/WARNINGS for format %s</strong></font>: <ul>%s</<ul>"
                )
                % (ecosio_format, Markup().join(report))
            )
        return response
