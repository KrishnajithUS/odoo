    ...
    * Official doc for EHF Billing 3.0 is the OpenPeppol BIS 3 doc +
    ...
    def _get_partner_party_legal_entity_vals_list(self, partner):
        ...
        for vals in vals_list:
            ...
            if partner.country_id.code == "LU":
                if (
                    "l10n_lu_peppol_identifier" in partner._fields
            ...
            if partner.country_code == "SE" and partner.company_registry:
                vals["company_id"] = "".join(
                    char for char in partner.company_registry if char.isdigit()
                )
    ...
    def _invoice_constraints_cen_en16931_ubl(self, invoice, vals):
        ...
        for role in ("supplier", "customer"):
            constraints[f"cen_en16931_{role}_country"] = self._check_required_fields(
                vals["vals"][f"accounting_{role}_party_vals"]["party_vals"][
                    "postal_address_vals"
                ]["country_vals"],
                "identification_code",
            ...
            if (
                ...
                and (
                    scheme_vals and scheme_vals[0]["tax_scheme_vals"].get("id") == "VAT"
                )
                and self._name
                in (
            ...
            constraints.update(
                {
                    ...
                    "nl_r_003": (
                        ...
                        if vals["supplier"].peppol_eas not in ("0106", "0190")
                        else ""
                    ),
                ...
                constraints.update(
                    {
                        ...
                        "nl_r_005": (
                            ...
                            if vals["customer"].commercial_partner_id.peppol_eas
                            not in ("0106", "0190")
                            else ""
                        ),
                    }
    ...
    def _import_retrieve_partner_vals(self, tree, role):
        ...
        endpoint_node = tree.find(
            f".//cac:Accounting{role}Party/cac:Party/cbc:EndpointID", UBL_NAMESPACES
        )
