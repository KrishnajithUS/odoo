...
class AccountMoveSend(models.TransientModel):
    ...
    checkbox_ubl_cii_xml = fields.Boolean(
        compute="_compute_checkbox_ubl_cii_xml", store=True, readonly=False
    )
    ...
    def _compute_checkbox_ubl_cii_label(self):
        for wizard in self:
            ...
            if wizard.mode in ("invoice_single", "invoice_multi"):
                code_to_label = dict(
                    wizard.move_ids.partner_id._fields["ubl_cii_format"].selection
                )
                codes = wizard.move_ids.partner_id.commercial_partner_id.mapped(
                    "ubl_cii_format"
                )
                if any(codes):
                    wizard.checkbox_ubl_cii_label = ", ".join(
                        code_to_label[c] for c in set(codes) if c
                    )

    @api.depends("move_ids")
    def _compute_enable_ubl_cii_xml(self):
        for wizard in self:
            wizard.enable_ubl_cii_xml = any(
                m._need_ubl_cii_xml() for m in wizard.move_ids
            )
    ...
    def _compute_checkbox_ubl_cii_xml(self):
        for wizard in self:
            wizard.checkbox_ubl_cii_xml = wizard.enable_ubl_cii_xml and (
                wizard.checkbox_ubl_cii_xml or wizard.company_id.invoice_is_ubl_cii
            )
    ...
    def _compute_ubl_warnings(self):
        for wizard in self:
            ...
            if not set(
                wizard.move_ids.partner_id.commercial_partner_id.mapped(
                    "ubl_cii_format"
                )
            ) - {False, "facturx", "oioubl_201"}:
                return

            wizard.show_ubl_company_warning = not (
                wizard.company_id.partner_id.peppol_eas
                and wizard.company_id.partner_id.peppol_endpoint
            )
            not_configured_partners = (
                wizard.move_ids.partner_id.commercial_partner_id.filtered(
                    lambda partner: not (partner.peppol_eas and partner.peppol_endpoint)
                )
            )
            if len(not_configured_partners) == 1:
                wizard.ubl_partner_warning = _(
                    "This partner is missing Peppol EAS or Peppol Endpoint field. "
