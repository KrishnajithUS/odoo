...
class Applicant(models.Model):
    ...
    email_normalized = fields.Char(
        index="trigram"
    )  # inherited via mail.thread.blacklist
    ...
    def _compute_application_count(self):
        ...
        if not any(self._ids):
            for applicant in self:
                ...
                if domain:
                    applicant.application_count = max(
                        0,
        ...
        self.flush_recordset(
            ["email_normalized", "partner_phone_sanitized", "partner_mobile_sanitized"]
        )
        self.env.cr.execute(
            """
            SELECT
        ...
        mapped_data = {
            result["id"]: result["similar_applicants"] for result in query_results
        }
        for applicant in self:
    ...
    def _get_similar_applicants_domain(self):
        ...
        if self.email_normalized:
            domain = expression.OR(
                [domain, [("email_normalized", "=", self.email_normalized)]]
            )
            ...
            domain = expression.OR(
                [
                    ...
                    [
                        ...
                        ("partner_mobile_sanitized", "=", self.partner_phone_sanitized),
                    ],
                ]
            )
                ...
                [
                    ...
                    [
                        ...
                        (
                            ...
                            "=",
                            self.partner_mobile_sanitized,
                        ),
                        ("partner_phone_sanitized", "=", self.partner_mobile_sanitized),
                    ],
                ]
            )
    ...
    def _inverse_partner_email(self):
        for applicant in self:
            ...
            if tools.email_normalize(applicant.email_from) != tools.email_normalize(
    ...
    def _compute_partner_mobile_sanitized(self):
        for applicant in self:
            applicant.partner_mobile_sanitized = (
                applicant._phone_format(fname="partner_mobile")
    ...
    def _email_is_blacklisted(self, mail):
        ...
        return normalized_mail in [
            m.strip()
            for m in self.env["ir.config_parameter"]
            .sudo()
            .get_param("hr_recruitment.blacklisted_emails", "")
            .split(",")
        ]
...
<p>%(para_1)s<br/>%(para_2)s</p>"""
    ...
    def get_view(self, view_id=None, view_type="form", **options):
        if (
            view_type == "form"
            and self.user_has_groups("hr_recruitment.group_hr_recruitment_interviewer")
            and not self.user_has_groups("hr_recruitment.group_hr_recruitment_user")
        ):
