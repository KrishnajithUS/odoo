    ...
    def _compute_applicant_emails(self):
        for wizard in self:
            wizard.applicant_emails = ", ".join(
                a.email_from for a in wizard.applicant_ids if a.email_from
            )
        ...
        if self.send_mail:
            ...
            if len(applicants) > 1:
                applicants.with_context(active_test=True).message_mail_with_source(
                    self.template_id, auto_delete_keep_log=True, **message_values
                )
            else:
                applicants.with_context(active_test=True).message_post_with_source(
                    self.template_id, subtype_xmlid="mail.mt_note", **message_values
                )
