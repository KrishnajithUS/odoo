...
class Attendee(models.Model):
    ...
    def _send_mail_to_attendees(self, mail_template, force_send=False):
        ...
        if mail_template.attachment_ids:
            ...
            attachments_values = [
                a.copy_data({"res_id": 0, "res_model": "mail.compose.message"})[0]
                for a in mail_template.attachment_ids
            ]
            attachments_values *= len(self)
            attendee_attachment_ids = (
                self.env["ir.attachment"].create(attachments_values).ids
            )
            ...
            attendee_id_attachment_id_map = dict(
                zip(
                    self.ids,
                    split_every(
                        template_attachment_count, attendee_attachment_ids, list
                    ),
                )
            )
        ...
        for attendee in self:
            if attendee.email and attendee._should_notify_attendee():
                ...
                attachment_ids = (
                    attendee_id_attachment_id_map[attendee.id]
                    if mail_template.attachment_ids
                    else []
                )
                ...
                body = mail_template._render_field(
                    "body_html", attendee.ids, compute_lang=True
                )[attendee.id]
