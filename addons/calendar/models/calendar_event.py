...
class Meeting(models.Model):
    ...
    def create(self, vals_list):
        ...
        vals_list = [  # Else bug with quick_create when we are filter on an other user
            (
                dict(vals, user_id=defaults.get("user_id", self.env.user.id))
    ...
    def read_group(
        ...
        fields_aggregates = [
            field_name
            for field_name in (fields or list(self._fields))
            if ":" in field_name
            or (field_name in self and self._fields[field_name].group_operator)
        ]
        grouped_fields = {
            group_field.split(":")[0] for group_field in groupby + fields_aggregates
        }
    ...
    def _create_videocall_channel_id(self, name, partner_ids):
        videocall_channel = self.env["discuss.channel"].create_group(
