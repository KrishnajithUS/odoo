...
class TestCalendar(SavepointCaseWithUserDemo):
    ...
    def test_event_creation_mail(self):
        ...
        def _test_emails_has_attachment(
            self, partners, attachments_names=["fileText_attachment.txt"]
        ):
        ...
        self.env.ref("calendar.calendar_template_meeting_invitation").attachment_ids = (
            attachments
        )
...
class TestCalendarTours(HttpCaseWithUserDemo):
    ...
    def test_calendar_decline_with_everybody_filter_tour(self):
        ...
        url = "/web#action=" + str(action_id.id) + "&view_type=calendar"
        self.start_tour(
            url, "test_calendar_decline_with_everybody_filter_tour", login="demo"
        )
        attendee = self.env["calendar.attendee"].search(
            [("event_id", "=", event.id), ("partner_id", "=", user_demo.partner_id.id)]
        )
