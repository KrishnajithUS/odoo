...
class Task(models.Model):
        ...
        for task in self:
            if task.allocated_hours > 0.0:
                task.remaining_hours_percentage = (
                    task.remaining_hours / task.allocated_hours
                )
            else:
    ...
    def _prepare_pattern_groups(self):
        return [
            self._get_group_pattern()["allocated_hours"]
    ...
    def _extract_allocated_hours(self):
        ...
        if self.allow_timesheets:
            self.allocated_hours = sum(
                float(num)
                for num in re.findall(allocated_hours_group, self.display_name)
            )
            self.display_name, dummy = re.subn(
                allocated_hours_group, "", self.display_name
            )
