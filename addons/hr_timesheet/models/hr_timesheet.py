...
class AccountAnalyticLine(models.Model):
    ...
    def _get_favorite_project_id(self, employee_id=False):
        last_timesheet_ids = self.search(
            self._get_favorite_project_id_domain(employee_id), limit=5
        )
    ...
    def _apply_timesheet_label(self, view_node, view_type="form"):
        ...
        for node in doc.xpath(
            ...
            node.set(
                "string", _("%s Spent", re.sub(r"[\(\)]", "", encoding_uom.name or ""))
            )
        return doc
    ...
    def _timesheet_preprocess(self, vals_list):
        ...
        if task_ids:
            ...
            account_ids = account_ids.union(
                tasks.analytic_account_id.ids, tasks.project_id.analytic_account_id.ids
            )
        ...
        uom_id_per_company = {
            company: company.project_time_mode_id.id for company in accounts.company_id
        }

        for index in timesheet_indices:
            vals = vals_list[index]
            data = (
                task_per_id[vals["task_id"]]
                if vals.get("task_id")
                else project_per_id[vals["project_id"]]
            )
            if not vals.get("project_id"):
                vals["project_id"] = data.project_id.id
            if not vals.get("account_id"):
                account = (
                    data._get_task_analytic_account_id()
                    if vals.get("task_id")
                    else data.analytic_account_id
                )
                if not account or not account.active:
                    raise ValidationError(
                        _(
                            "Timesheets must be created on a project or a task with an active analytic account."
                        )
                    )
            ...
            if not vals.get("product_uom_id"):
                company = (
                    account_per_id[vals["account_id"]].company_id or data.company_id
                )
                vals["product_uom_id"] = (
                    uom_id_per_company.get(company.id, company.project_time_mode_id.id)
                    or self.env.company.project_time_mode_id.id
                )
