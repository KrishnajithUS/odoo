...
class HrEmployeDeleteWizard(models.TransientModel):
    ...
    employee_ids = fields.Many2many(
        "hr.employee", string="Employees", context={"active_test": False}
    )
    has_active_employee = fields.Boolean(
        string="Has Active Employee", compute="_compute_has_active_employee"
    )
    has_timesheet = fields.Boolean(
        string="Has Timesheet", compute="_compute_has_timesheet", compute_sudo=True
    )
