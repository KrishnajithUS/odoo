...
class IrModule(models.Model):
    ...
    def _import_module(self, module, path, force=False, with_demo=False):
        ...
        if unmet_dependencies:
            if unmet_dependencies == set(["web_studio"]) and _is_studio_custom(path):
                ...
                to_install = known_mods.filtered(
        ...
        for kind in kind_of_files:
            for filename in terp.get(kind, []):
                ...
                with file_open(pathname, "rb", env=self.env) as fp:
                    if ext == ".csv":
                        convert_csv_import(
                            self.env, module, pathname, fp.read(), idref, mode, noupdate
                        )
                    elif ext == ".sql":
    ...
    def web_search_read(
        ...
        if _domain_asks_for_industries(domain):
            ...
            modules_list = self._get_modules_from_apps(
                fields_name, "industries", False, domain, offset=offset, limit=limit
            )
            return {
    ...
    def button_immediate_install_app(self):
        ...
        try:
            ...
            raise UserError(
                _(
                    "Connection to %s failed, the module %s cannot be downloaded.",
                    APPS_URL,
                    module_name,
                )
            )
        ...
        with zipfile.ZipFile(BytesIO(zip_data), "r") as z:
            ...
            for manifest_file in manifest_files:
                ...
                dependencies_to_install |= known_mods.filtered(
                    lambda m: m.name in unmet_dependencies
                )
                not_found_modules |= set(
                    mod
