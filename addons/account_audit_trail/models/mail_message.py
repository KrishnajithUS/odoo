...
class Message(models.Model):
    ...
    account_audit_log_preview = fields.Html(
        string="Description", compute="_compute_account_audit_log_preview"
    )
    ...
    account_audit_log_display_name = fields.Char(
        compute="_compute_account_audit_log_display_name"
    )
    show_audit_log = fields.Boolean(
        compute="_compute_show_audit_log", search="_search_show_audit_log"
    )

    def _compute_account_audit_log_preview(self):
        for message in self:
            title = message.subject or message.preview
            tracking_value_ids = message.sudo().tracking_value_ids.filtered(
                ...
                [
                    ...
                    (
                        ...
                        self.env["account.move"]._search(
                            [
                                (
                                    "company_id.check_account_audit_trail",
                                    operator,
                                    value,
                                ),
                            ]
    ...
    def _compute_audit_log_related_record_id(self, model, fname, domain):
        ...
        if messages_of_related:
            related_recs = (
                self.env[model]
                .sudo()
                .search([("id", "in", messages_of_related.mapped("res_id"))] + domain)
            )
    ...
    def _search_audit_log_related_record_id(self, model, operator, value):
        if operator in [
            ...
            res_id_domain = [
                ("res_id", "in", self.env[model]._name_search(value, operator=operator))
            ]
