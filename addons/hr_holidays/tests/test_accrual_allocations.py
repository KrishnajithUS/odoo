...
class TestAccrualAllocations(TestHrHolidaysCommon):
    ...
    def test_unused_accrual_lost(self):
        ...
        with freeze_time("2022-01-01"):
            allocation._update_accrual()
            self.assertEqual(
                allocation.number_of_days, 0, "The number of days should be reset"
            )
    ...
    def test_accrual_carryover_at_allocation(self):
        ...
        self.assertEqual(
            allocation.number_of_days,
            0,
            "Should accrue 0 days, days are added on 27th.",
        )
        ...
        with freeze_time("2024-04-27"):
            allocation._update_accrual()
        self.assertAlmostEqual(
            allocation.number_of_days, 1.03, 2, "Allocations lost, then 1 accrued."
        )
    ...
    def test_added_type_during_onchange(self):
        ...
        res = self.env["hr.leave.accrual.level"].onchange(
            {"accrual_plan_id": {"id": accrual_plan.id}}, [], {"added_value_type": {}}
        )
        self.assertEqual(
            res["value"]["added_value_type"], accrual_plan.level_ids[0].added_value_type
        )
    ...
    def test_future_accural_time_with_leaves_taken_in_the_past(self):
        ...
        with freeze_time("2024-03-01"):
            ...
            self.assertEqual(
                get_reamining_leaves(2024, 3, 10),
                10,
