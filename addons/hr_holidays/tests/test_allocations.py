...
class TestAllocations(TestHrHolidaysCommon):
    ...
    def test_allocation_whole_company(self):
        ...
        num_of_allocations = self.env["hr.leave.allocation"].search_count(
            [("employee_id", "=", self.employee.id), ("multi_employee", "=", False)]
        )
    ...
    def test_allocation_request_day(self):
        self.leave_type.write(
            {"name": "Custom Time Off Test", "allocation_validation_type": "officer"}
        )
        ...
        self.assertEqual(
            employee_allocation.private_name, "Custom Time Off Test (10.0 day(s))"
        )
    ...
    def test_allocation_dropdown_after_period(self):
        ...
        self.assertEqual(
            result[0][1], "Compensatory Days (72 remaining out of 72 hours)"
        )
