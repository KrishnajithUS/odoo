...
class HolidaysRequest(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        if "state" in fields_list and not defaults.get("state"):
            defaults["state"] = (
                "confirm" if lt.leave_validation_type != "no_validation" else "draft"
            )
    ...
    def _default_get_request_dates(self, values):
        ...
        if values.get("date_to"):
            if not values.get("request_date_to"):
                values["request_date_to"] = pytz.utc.localize(
                    values["date_to"]
                ).astimezone(client_tz)
            del values["date_to"]
        return values
    ...
    is_user_only_responsible = fields.Boolean(
    ...
    leave_type_increases_duration = fields.Boolean(
    ...
    def _compute_date_from_to(self):
        for holiday in self:
            if not holiday.request_date_from:
                ...
                if (
                    holiday.request_unit_half or holiday.request_unit_hours
                ) and holiday.request_date_to != holiday.request_date_from:
                    holiday.request_date_to = holiday.request_date_from

                day_period = (
                    {"am": "morning", "pm": "afternoon"}.get(
                        holiday.request_date_from_period, None
                    )
                ...
                holiday.date_to = self._to_utc(
                    compensated_request_date_to, hour_to, holiday.employee_id or holiday
                )
    ...
    def _compute_is_user_only_responsible(self):
        user = self.env.user
        self.is_user_only_responsible = user.has_group(
    ...
    def _compute_leave_type_increases_duration(self):
        for holiday in self:
            ...
            holiday.leave_type_increases_duration = (
                holiday.leave_type_request_unit == "day"
                and days < holiday.number_of_days
            )
        ...
        for holiday in self:
            ...
            if conflicting_holidays:
                ...
                for conflicting_holiday in conflicting_holidays:
                    ...
                    conflicting_holiday_data["employee_name"] = (
...
""",
        ...
        for leave in self:
            ...
            if self.env.context.get("short_name"):
                ...
                if leave.leave_type_request_unit == "hour":
                    ...
                    leave.display_name = _(
                        "%s: %.2f days", short_leave_name, leave.number_of_days
                    )
            else:
                ...
                if leave.leave_type_request_unit == "hour":
                    if self.env.context.get(
                        ...
                        leave.display_name = _(
                            "%(leave_type)s: %(duration).2f hours on %(date)s",
                        ...
                        leave.display_name = _(
                            "%(person)s: %(duration).2f hours on %(date)s",
                            person=target,
                            duration=leave.number_of_hours_display,
                            date=display_date,
                        )
                    ...
                    if (
                        ...
                        leave.display_name = _(
                            "%(person)s: %(duration).2f days (%(start)s)",
                            person=target,
                            duration=leave.number_of_days,
                            start=display_date,
                        )
    ...
    def _check_mandatory_day(self):
        ...
        if not is_leave_user and any(leave.has_mandatory_day for leave in self):
            raise ValidationError(
                _("You are not allowed to request a time off on a Mandatory Day.")
            )
    ...
    def unlink(self):
        self._force_cancel(
            _("deleted by %s (uid=%d).", self.env.user.display_name, self.env.user.id)
        )
        return super(
    ...
    def _split_leaves(self, split_date_from, split_date_to):
        ...
        multi_day_leaves = self.filtered(
            lambda l: l.request_date_from < split_date_from
    ...
    def _notify_manager(self):
        leaves = self.filtered(
            lambda hol: (
                hol.validation_type == "both" and hol.state in ["validate1", "validate"]
            )
        ...
        for holiday in self:
            if holiday.state == "draft":
                ...
                if holiday.holiday_status_id.leave_validation_type != "no_validation":
                    ...
                    user_ids = (
                        holiday.sudo()._get_responsible_for_approval().ids
                        or self.env.user.ids
                    )
