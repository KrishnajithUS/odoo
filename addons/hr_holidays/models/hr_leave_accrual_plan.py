...
class AccrualPlan(models.Model):
    ...
    level_ids = fields.One2many(
        "hr.leave.accrual.level", "accrual_plan_id", copy=True, string="Milestone"
    )
    ...
    carryover_day_display = fields.Selection(
        _get_selection_days,
        compute="_compute_carryover_day_display",
        inverse="_inverse_carryover_day_display",
    )
    ...
    def _inverse_carryover_day_display(self):
        for plan in self:
            if plan.carryover_day_display == "last":
                ...
                plan.carryover_day = (
                    DAY_SELECT_VALUES.index(plan.carryover_day_display) + 1
                )
