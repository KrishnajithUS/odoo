    ...
    def write(self, values):
        ...
        if "resource_calendar_id" in values and not self.env.context.get(
            ...
            try:
                ...
                ).write({"resource_calendar_id": values["resource_calendar_id"]})
...
class HrEmployee(models.Model):
    ...
    def _is_leave_user(self):
        return self == self.env.user.employee_id and self.user_has_groups(
            "hr_holidays.group_hr_holidays_user"
        )
    ...
    def get_public_holidays_data(self, date_start, date_end):
        ...
        employee_tz = pytz.timezone(
            self._get_tz() if self else self.env.user.tz or "utc"
        )
    ...
    def get_mandatory_days_data(self, date_start, date_end):
        self = self._get_contextual_employee()
        mandatory_days = self._get_mandatory_days(date_start, date_end).sorted(
            "start_date"
        )
        return list(
            map(
