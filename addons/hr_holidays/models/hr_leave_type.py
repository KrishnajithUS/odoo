...
class HolidaysType(models.Model):
    ...
    def _search_valid(self, operator, value):
        ...
        if {"default_date_from", "default_date_to", "tz"} <= set(self._context):
            default_date_from_dt = fields.Datetime.to_datetime(
                self._context.get("default_date_from")
            )
            default_date_to_dt = fields.Datetime.to_datetime(
                self._context.get("default_date_to")
            )
    ...
    def _compute_leaves(self):
        employee = self.env["hr.employee"]._get_contextual_employee()
        target_date = (
            self._context["default_date_from"]
        ...
        if target_date:
            self.env.context = frozendict(
                self.env.context, leave_date_from=self._context["default_date_from"]
            )
        else:
    ...
    def requested_display_name(self):
        ...
        ) and self._context.get("employee_id")

    @api.depends(
        "requires_allocation", "virtual_remaining_leaves", "max_leaves", "request_unit"
    )
    @api.depends_context(
        "holiday_status_display_name", "employee_id", "from_manager_leave_form"
    )
    ...
    def _search(self, domain, offset=0, limit=None, order=None, access_rights_uid=None):
        ...
        if order == self._order and employee:
            ...
            leaves = self.browse(
                super()._search(domain, access_rights_uid=access_rights_uid)
            )
    ...
    def get_allocation_data(self, employees, target_date=None):
        ...
        leave_type_requires_allocation = self.filtered(
            lambda lt: lt.requires_allocation == "yes"
        )
        ...
        for employee in employees:
            for leave_type in leave_type_requires_allocation:
                ...
                for closest_allocation in closest_allocations:
                    closest_allocation_remaining += allocations_leaves_consumed[
                        employee
                    ][leave_type][closest_allocation]["virtual_remaining_leaves"]
                if closest_allocation.date_to:
                    ...
                    closest_allocation_duration = (
                        calendar._attendance_intervals_batch(
                            datetime.combine(
                                closest_allocation.date_to, time.min
                            ).replace(tzinfo=pytz.UTC),
                            datetime.combine(target_date, time.max).replace(
                                tzinfo=pytz.UTC
                            ),
                        )
