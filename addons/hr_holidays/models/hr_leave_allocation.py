...
class HolidaysAllocation(models.Model):
    ...
    state = fields.Selection(
        [("confirm", "To Approve"), ("refuse", "Refused"), ("validate", "Approved")],
        string="Status",
        readonly=True,
        tracking=True,
        copy=False,
    ...
    has_accrual_plan = fields.Boolean(
    ...
    def _compute_leaves(self):
        date_from = (
            fields.Date.from_string(self._context["default_date_from"])
            if "default_date_from" in self._context
            else fields.Date.today()
        )
        employee_days_per_allocation = self.employee_id._get_consumed_leaves(
            self.holiday_status_id, date_from, ignore_future=True
        )[0]
        for allocation in self:
            ...
            allocation.leaves_taken = employee_days_per_allocation[origin.employee_id][
        ...
        for allocation in self:
            ...
            if allocation_unit != "hour":
                ...
                allocation.number_of_days = (
                    allocation.number_of_hours_display / hours_per_day
                )
    ...
    def _get_carryover_date(self, date_from):
        ...
        if carryover_time == "year_start":
            ...
            carryover_date = date(
                date_from.year,
                MONTHS_TO_INTEGER[accrual_plan.carryover_month],
                accrual_plan.carryover_day,
            )
    ...
    def _add_days_to_allocation(
        ...
        if current_level.cap_accrued_time:
            self.number_of_days = min(
                self.number_of_days, current_level_maximum_leave + leaves_taken
            )
    ...
    def _process_accrual_plans(self, date_to=False, force_period=False, log=True):
        ...
        for allocation in self:
            ...
            leaves_taken = (
                ...
                if first_level.added_value_type == "day"
            ...
            while allocation.nextcall <= date_to:
                ...
                if allocation.nextcall == carryover_date:
                    if current_level.action_with_unused_accruals in ["lost", "maximum"]:
                        ...
                        allocation_max_days = (
                            current_level.postpone_max_days + leaves_taken
                        )
                        allocation.number_of_days = min(
                            allocation_days, allocation_max_days
                        )
    ...
    def _compute_display_name(self):
        for allocation in self:
            if allocation.holiday_type == "company":
                ...
                target = _(
                    "%(first)s, %(second)s and %(amount)s others",
        ...
        for allocation in allocations:
            ...
            if allocation.validation_type == "officer":
                partners_to_subscribe.add(
                    allocation.employee_id.parent_id.user_id.partner_id.id
                )
                partners_to_subscribe.add(
        ...
        if (
            ...
            for allocation in self:
                ...
                total_current_excess = sum(
                    leave_date["amount"] for leave_date in current_excess.values()
                )
                total_previous_excess = sum(
                    leave_date["amount"] for leave_date in previous_excess.values()
                )
                ...
                if (
                    lt.allows_negative
                    and total_current_excess <= lt.max_allowed_negative
                ):
                    continue
    ...
    def action_validate(self):
        ...
        if to_validate:
            to_validate.write(
                {"state": "validate", "approver_id": self.env.user.employee_id.id}
            )
    ...
    def _action_validate_create_childs(self):
        ...
        for allocation in self:
            ...
            if allocation.linked_request_ids:
                continue
            if allocation.state == "validate" and (
                allocation.holiday_type in ["category", "department", "company"]
                or (
                    allocation.holiday_type == "employee"
                    and len(allocation.employee_ids) > 1
                )
            ):
                if allocation.holiday_type == "employee":
        ...
        self.linked_request_ids.filtered(
            lambda c: c.state != "validate"
        ).action_validate()
    ...
    def action_refuse(self):
        ...
        days_per_allocation = self.employee_id._get_consumed_leaves(
            self.holiday_status_id
        )[0]

        for allocation in self:
            days_taken = days_per_allocation[allocation.employee_id][
                allocation.holiday_status_id
    ...
    def _check_approval_update(self, state):
        ...
        for allocation in self:
            ...
            if (
                not is_officer
                and self.env.user != allocation.employee_id.leave_manager_id
                and not val_type == "no"
            ):
                raise UserError(
                    _(
                            ...
                            activity_vals.append(
                                {
                                    ...
                                    "res_model_id": self.env.ref(
                                        "hr_holidays.model_hr_leave_allocation"
                                    ).id,
                                }
                            )
