...
class HrAttendance(http.Controller):
        ...
        if employee:
            response = {
                **HrAttendance._get_user_attendance_data(employee),
                ...
                "overtime_today": request.env["hr.attendance.overtime"]
                .sudo()
                .search(
                    [
    ...
    def kiosk_menu_item_action(self):
        ...
        if request.env.user.user_has_groups(
            ...
            employee_list = [
                {
                    ...
                    "department": {
                        "id": e["department_id"][0] if e["department_id"] else False,
                        "name": e["department_id"][1] if e["department_id"] else False,
                    },
                }
                for e in request.env["hr.employee"]
                .sudo()
                .search_read(
                    domain=[("company_id", "=", company.id)],
                    fields=["id", "name", "avatar_256", "job_id", "department_id"],
                )
            ]
            departement_list = [
