                ...
                try:
                    ...
                    if lead_emails:
                        try:
                            ...
                            _logger.info(
                                "Lead enrichment failed because of insufficient credit"
                            )
                            if not from_cron:
                            ...
                            if not from_cron:
                                self.env["iap.account"]._send_error_notification(
                                    message=_(
                                        "An error occurred during lead enrichment"
                                    )
                                )
                            _logger.info(
                                "An error occurred during lead enrichment: %s", e
                            )
                        else:
                            if not from_cron:
                            ...
                            _logger.info(
                                "Batch of %s leads successfully enriched",
                                len(lead_emails),
                            )
