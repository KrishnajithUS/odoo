...
class TestWorkEntryLeave(TestWorkEntryHolidaysBase):
    ...
    def test_write_mark_conflicting_work_entries(self):
        ...
        work_entry = self.create_work_entry(
            leave.date_from - relativedelta(days=1), leave.date_from
        )  # the day before
        self.assertNotEqual(
            work_entry.state, "conflict", "It should not be conflicting"
        )
    ...
    def test_refuse_leave(self):
        ...
        work_entries = self.richard_emp.contract_id._generate_work_entries(
            datetime(2019, 10, 10, 0, 0, 0), datetime(2019, 10, 10, 23, 59, 59)
        )
        adjacent_work_entry = self.create_work_entry(
            leave.date_from - relativedelta(days=3), leave.date_from
        )
        self.assertTrue(
    ...
    def test_refuse_approved_leave(self):
        ...
        leave_work_entry = self.richard_emp.contract_ids.generate_work_entries(
            start.date(), end.date()
        )
    ...
    def test_work_entry_cancel_leave(self):
        ...
        with freeze_time(datetime(2022, 3, 21)):
            ...
            self.richard_emp.contract_ids.generate_work_entries(
