...
class Employee(models.Model):
    ...
    license_plate = fields.Char(
        compute="_compute_license_plate", search="_search_license_plate"
    )
        ...
        for employee in self:
            if employee.private_car_plate and employee.car_ids.license_plate:
                employee.license_plate = " ".join(
                    [employee.car_ids.license_plate, employee.private_car_plate]
                )
            else:
                employee.license_plate = (
                    employee.car_ids.license_plate or employee.private_car_plate
                )
            ...
            vehicles = self.env["fleet.vehicle"].search(
                [
                    (
                        "driver_id",
                        "in",
                        (self.user_id.partner_id | self.sudo().work_contact_id).ids,
                    )
                ]
            )
    ...
    def _sync_employee_cars(self, user):
        if self.work_contact_id and self.work_contact_id != user.partner_id:
            cars = self.env["fleet.vehicle"].search(
                [
                    "|",
                    ("future_driver_id", "=", self.work_contact_id.id),
                    ("driver_id", "=", self.work_contact_id.id),
