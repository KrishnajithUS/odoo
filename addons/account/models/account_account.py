...
class AccountAccount(models.Model):
    ...
    code = fields.Char(
        size=64, required=True, tracking=True, index=True, unaccent=False
    )
    ...
    def _load_precommit_update_opening_move(self):
        ...
        for company, company_accounts in accounts_per_company.items():
            company._update_opening_move(
                {account: data[account.id] for account in company_accounts}
            )
...
class AccountGroup(models.Model):
    ...
    def _adapt_accounts_for_account_groups(self, account_ids=None, company=None):
        ...
        if account_ids:
            account_where_clause = SQL(
                "%s AND account.id IN %s", account_where_clause, tuple(account_ids)
            )
