...
class SequenceMixin(models.AbstractModel):
    ...
    def _year_match(self, format_value, date):
        return format_value == self._truncate_year_to_length(
            date.year, len(str(format_value))
        )
    ...
    def _sequence_matches_date(self):
        ...
        year_match = (
            ...
            or self._year_match(format_values["year"], year_start)
        ) and (
            not format_values["year_end"]
            or self._year_match(format_values["year_end"], year_end)
        )
    ...
    def _set_next_sequence(self):
        ...
        if new:
            ...
            format_values["year_end"] = self._truncate_year_to_length(
                date_end.year, format_values["year_end_length"]
            )
