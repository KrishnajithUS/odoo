...
class AccountChartTemplate(models.AbstractModel):
    ...
    def _setup_complete(self):
        ...
        self.env.registry[self._name]._template_register = (
            AccountChartTemplate._template_register
        )
    ...
    def try_loading(self, template_code, company, install_demo=True):
        ...
        if template_code == "syscohada" and template_code != company.chart_template:
            raise UserError(
                _(
                    "The Syscohada chart template shouldn't be selected directly. Instead, you should directly select the chart template related to your country."
                )
            )
    ...
    def _load(self, template_code, company, install_demo):
        ...
        if module:
            ...
            ]  # create a new env with the new registry
        ...
        if not reload_template and (
            not company.root_id._existing_accounting()
            or self.env.ref("base.module_account").demo
        ):
            for model in ("account.move",) + TEMPLATE_MODELS[::-1]:
                if not company.parent_id:
                    ...
                    ).with_context({MODULE_UNINSTALL_FLAG: True}).unlink()
        ...
        if (
            install_demo
            and self.ref("base.module_account").demo
            and not reload_template
        ):
            try:
    ...
    def _pre_reload_data(self, company, template_data, data):
        ...
        xmlid2tax = {
            xml_id.split(".")[1]
            .split("_", maxsplit=1)[1]: self.env["account.tax"]
            .browse(record)
            ...
            for xmlid, values in records.items():
                if model_name == "account.fiscal.position":
                            ...
                            match element:
                                case (
                                    ...
                                    or (
                                        dest_id
                                        and not self.ref(
                                            dest_id, raise_if_not_found=False
                                        )
                                    )
                                ):
                            ...
                            for element in values.get("repartition_line_ids", []):
                                match element:
                                    case (
                                        int() as command,
                                        _,
                                        {"tag_ids": tags} as repartition_line_values,
                                    ) if command in tuple(Command):
                    ...
                    if not account or not re.match(
                        ...
                        query.add_where(
                            "account_account.code SIMILAR TO %s",
                            [f'{values["code"]}0*'],
                        )
    ...
    def _post_load_data(self, template_code, company, template_data):
        ...
        for journal in [
            self.ref(kind, raise_if_not_found=False) for kind in ("bank", "cash")
        ]:
            if journal:
        ...
        if sale_journal and template_data.get("property_account_income_categ_id"):
            sale_journal.default_account_id = self.ref(
                template_data.get("property_account_income_categ_id")
            )
        ...
        if purchase_journal and template_data.get("property_account_expense_categ_id"):
            purchase_journal.default_account_id = self.ref(
                template_data.get("property_account_expense_categ_id")
            )
        ...
        if not company.parent_id and self.env["account.tax"].search(
            ...
            **additional_properties,
            ...
            if value and field in self.env[model]._fields:
                self.env["ir.property"]._set_default(
                    field, model, self.ref(value).id, company=company
                )
    ...
    def _instantiate_foreign_taxes(self, country, company):
        ...
        for tax_template in tax_data.values():
            ...
            account_id = [
                rep_line["account_id"]
                for _command, _id, rep_line in tax_template["repartition_line_ids"]
                if rep_line.get("account_id")
            ]
            if account_id:
                local_account = self.env["account.account"].browse(
                    existing_accounts[account_id[0]]
                )
                existing_accounts[account_xml_id] = create_foreign_tax_account(
                    local_account, _("Cash basis transition account")
                ).id
                continue
    ...
    def ref(self, xmlid, raise_if_not_found=True):
        ...
        return self.env.ref(
            f"account.{self.env.company.id}_{xmlid}", raise_if_not_found=False
        ) or self.env.ref(
            f"account.{self.env.company.parent_ids[0].id}_{xmlid}", raise_if_not_found
        )
            ...
            for tag in args:
                if re.match(r"^\w+\.\w+$", tag):
                    ...
                    if not mapped_tag:
                        raise UserError(
                            _(
                                "Error while loading the localization. You should probably update your localization app first."
                            )
                        )
    ...
    def _get_untranslated_translatable_template_model_records(self, langs, companies):
        ...
        for model in TEMPLATE_MODELS:
            ...
            self.env[model].flush_model(
                ["id", "company_id"] + translatable_model_fields[model]
            )
            ...
            for field in translatable_fields:
                translatable_field_column_args.extend(
                    (SQL("%s", field), SQL.identifier("model", field))
                )
    ...
    def _load_translations(self, langs=None, companies=None, template_data=None):
        ...
        for chart_template, chart_companies in groupby(
            companies, lambda c: c.chart_template
        ):
            template_data = template_data or self.env[
                "account.chart.template"
                            ...
                            if field_translation:
                                for company in chart_companies:
                                    xml_id = (
                                        _xml_id
                                        if "." in _xml_id
                                        else f"account.{company.id}_{_xml_id}"
                                    )
                                    translation_importer.model_translations[mname][
                                        fname
                                    ][xml_id][lang] = field_translation
