...
class AccountTax(models.Model):
    ...
    def _message_log_repartition_lines(self, old_values_str, new_values_str):
        ...
        added_and_deleted_lines = [
            (
                (line, _("Removed"), old_line_values_dict[line])
                if line in old_line_values_dict
                else (line, _("New"), new_line_values_dict[line])
            )
            for line in old_line_values_dict.keys() ^ new_line_values_dict.keys()
        ]
    ...
    def _compute_display_name(self):
        ...
        for record in self:
            ...
            if (
                record.country_id
                != record.company_id._accessible_branches()[
                    :1
    ...
    def compute_all(
        ...
        def recompute_base(base_amount, incl_tax_amounts):
            ...
            percent_amount = sum(
                ...
                for _i, tax_amount, sum_repartition_factor in incl_tax_amounts[
                    "percent_taxes"
                ]
            )
            ...
            if company.country_code == "IN":
                ...
                for i, tax_amount, sum_repartition_factor in incl_tax_amounts[
                    "percent_taxes"
                ]:
                    gross_tax_amount = float_round(
                        base * tax_amount / (100 + percent_amount),
                        precision_rounding=prec,
                    )
                    factored_tax_amount = float_round(
                        gross_tax_amount * sum_repartition_factor,
                        precision_rounding=prec,
                    )
                    total_tax_amount += factored_tax_amount
                    cached_tax_amounts[i] = gross_tax_amount
                    fixed_amount += factored_tax_amount
                for i, _tax_amount, _sum_repartition_factor in incl_tax_amounts[
                    "percent_taxes"
                ]:
    ...
    def _get_generation_dict_from_base_line(
        ...
        return {
            ...
            "analytic_distribution": (
                line_vals["analytic_distribution"] if tax_vals["analytic"] else {}
            ),
            "_extra_grouping_key_": line_vals.get("extra_context", {}).get(
                "_extra_grouping_key_"
            ),
        }
    ...
    def _get_generation_dict_from_tax_line(self, line_vals):
        ...
        return {
            ...
            "analytic_distribution": (
                line_vals["analytic_distribution"] if tax.analytic else {}
            ),
            "_extra_grouping_key_": line_vals.get("extra_context", {}).get(
                "_extra_grouping_key_"
            ),
    ...
    def _prepare_tax_totals(
        ...
        for tax_detail in global_tax_details["tax_details"].values():
            tax_group_vals = {
                ...
                "hide_base_amount": all(
                    x["tax_repartition_line"].tax_id.amount_type == "fixed"
                    for x in tax_detail["group_tax_details"]
                ),
            }
        ...
        result = {
            "amount_untaxed": (
                currency.round(amount_untaxed) if currency else amount_untaxed
            ),
        ...
        if is_company_currency_requested:
            comp_currency = self.env.company.currency_id
            result["amount_total_company_currency"] = comp_currency.round(
                amount_total_company_currency
            )
    ...
    @api.model
    def _dispatch_negative_lines(
        self, base_lines, sorting_criteria=None, additional_dispatching_method=None
    ):
        """
        ...
        for neg_base_line in list(results["orphan_negative_lines"]):
            ...
            sorting_criteria = (
                sorting_criteria
                or self._get_negative_lines_sorting_candidate_criteria()
            )
            ...
            for candidate in sorted_candidates:
                net_price_subtotal = (
                    neg_base_line["gross_price_subtotal"]
                    - neg_base_line["discount_amount"]
                )
                other_net_price_subtotal = (
                    candidate["gross_price_subtotal"] - candidate["discount_amount"]
                )
                discount_to_distribute = min(
                    other_net_price_subtotal, -net_price_subtotal
                )
                ...
                if additional_dispatching_method:
                    additional_dispatching_method(
                        neg_base_line=neg_base_line,
                        candidate=candidate,
                        discount_to_distribute=discount_to_distribute,
                        is_zero=is_zero,
                    )

                # Check if there is something left on the other line.
                remaining_amount = (
                    candidate["discount_amount"] - candidate["gross_price_subtotal"]
                )
        ...
        def same_price_subtotal(candidate, negative_line):
            return (
                candidate["currency"].compare_amounts(
                    candidate["price_subtotal"], -negative_line["price_subtotal"]
                )
                != 0
            )
...
class AccountTaxRepartitionLine(models.Model):
    ...
    tax_id = fields.Many2one(
        comodel_name="account.tax", ondelete="cascade", check_company=True
    )
    company_id = fields.Many2one(
        string="Company",
