...
class AccountFiscalPosition(models.Model):
            ...
            if record.foreign_vat:
                ...
                if record.country_group_id:
                    ...
                    if not foreign_vat_country:
                        raise ValidationError(
                            _(
                                "The country code of the foreign VAT number does not match any country in the group."
                            )
                        )
                    similar_fpos_domain += [
                        ("country_group_id", "=", record.country_group_id.id),
    ...
    def _get_fpos_by_region(
        ...
        if fpos := local_get_fpos_by_region(
            [("company_id", "=", self.env.company.id)] + base_domain
        ):
            return fpos
        else:
    ...
    def _get_fiscal_position(self, partner, delivery=None):
        ...
        fp = self._get_fpos_by_region(
            delivery.country_id.id, delivery.state_id.id, delivery.zip, vat_valid
        )
    ...
    def action_create_foreign_taxes(self):
        ...
        self.env["account.chart.template"]._instantiate_foreign_taxes(
        ...
        if not partner:
            ...
            if vat_only_numeric:
                ...
                query = self.env["res.partner"]._search(
                    extra_domain + [("active", "=", True)], limit=2
                )
                query.add_where(
                    "res_partner.vat ~ %s",
