...
class ResPartnerBank(models.Model):
    ...
    def _compute_user_has_group_validate_bank_account(self):
        user_has_group_validate_bank_account = self.user_has_groups(
    ...
    def _build_qr_code_vals(
        ...
        for candidate_method, candidate_name in candidate_methods:
            error_msg = self._get_error_messages_for_qr(
    ...
    def _get_qr_code_url(
        ...
        return (
            "/report/barcode/?" + werkzeug.urls.url_encode(params) if params else None
        )
    ...
    def write(self, vals):
        ...
        if "allow_out_payment" in vals and not self.user_has_groups(
            "account.group_validate_bank_account"
        ):
            raise UserError(
