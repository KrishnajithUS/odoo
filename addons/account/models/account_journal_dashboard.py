...
class account_journal(models.Model):
    ...
    def _kanban_dashboard_graph(self):
        ...
        (self - bank_cash_journals - sale_purchase_journals).kanban_dashboard_graph = (
            False
        )
    ...
    def _query_has_sequence_holes(self):
        self.env["account.move"].flush_model(
            ["journal_id", "date", "sequence_prefix", "sequence_number", "state"]
        )
        queries = []
        for company in self.env.companies:
            queries.append(
                ...
                """,
                    ...
                    fiscalyear_lock_date_clause=(
                        SQL("move.date > %s", lock_date)
                        if (lock_date := company.fiscalyear_lock_date)
                        else SQL("TRUE")
                    ),
                )
            )
    ...
    def _get_bank_cash_graph_data(self):
        ...
        """
        self.env.cr.execute(
            query, (self.ids, last_month, today, self.env.companies.ids)
        )
            ...
            dashboard_data[journal.id].update(
                {
                    ...
                    "misc_operations_balance": (
                        currency.format(misc_balance) if currency_consistent else None
                    ),
                }
            )
    ...
    def _fill_sale_purchase_dashboard_data(self, dashboard_data):
        ...
        if purchase_journals:
            query, params = purchase_journals._get_open_payments_query().select(
                *payment_field_list
            )
            self.env.cr.execute(query, params)
            query_results_payments_to_pay = group_by_journal(self.env.cr.dictfetchall())
            for journal in purchase_journals:
                query_results_to_pay[journal.id] = query_results_payments_to_pay[
                    journal.id
                ]
        if sale_journals:
            query, params = sale_journals._get_open_bills_to_pay_query().select(
                *bills_field_list
            )
            self.env.cr.execute(query, params)
            query_results_bills_to_pay = group_by_journal(self.env.cr.dictfetchall())
            for journal in sale_journals:
                query_results_to_pay[journal.id] = query_results_bills_to_pay[
                    journal.id
                ]
        ...
        late_query_results = {}
        if purchase_journals:
            query, params = purchase_journals._get_late_payment_query().select(
                *payment_field_list
            )
            self.env.cr.execute(query, params)
        ...
        if sale_journals:
            query, params = sale_journals._get_late_bills_query().select(
                *bills_field_list
            )
            self.env.cr.execute(query, params)
        ...
        to_check_vals = {
            ...
            for journal, amount_total_signed_sum, count in self.env[
                "account.move"
            ]._read_group(
                domain=[
                    *self.env["account.move"]._check_company_domain(self.env.companies),
        ...
        for result in results_dict:
            ...
            if company.currency_id == target_currency:
                ...
                total_amount += document_currency._convert(
                    result.get("amount_total"), target_currency, company, date
                )
