...
class AccountMove(models.Model):
    ...
    def _compute_tax_totals(self):
        ...
        for move in self:
            if move.is_invoice(include_receipts=True):
                ...
                kwargs["is_company_currency_requested"] = (
                    ...
                    if rounding_amount:
                        if move.invoice_cash_rounding_id.strategy == "add_invoice_line":
                            ...
                            if totals["subtotals_order"]:
                                ...
                                max_tax_group["formatted_tax_group_amount"] = (
                                    formatLang(
                                        self.env,
        ...
        for move in self:
            ...
            if show_warning:
                total_field = (
                    ...
                    else "amount_total_company_currency"
                )
    ...
    def _prepare_invoice_aggregated_taxes(
        ...
        for tax_line in tax_lines:
            tax_rep_amounts = current_tax_amount_per_rep_line.setdefault(
        ...
        for _base_line, _to_update_vals, tax_values_list in to_process:
            for tax_values in tax_values_list:
                ...
                tax_rep_amounts = tax_details_per_rep_line.setdefault(
                    tax_rep_id,
                    {
                        "tax_amount_currency": 0.0,
                        "tax_amount": 0.0,
                ...
                tax_rep_amounts["tax_amount_currency"] += tax_values[
                    "tax_amount_currency"
                ]
        ...
        for key, currency in (
            ...
            for (
                tax_rep_id,
                computed_tax_rep_amounts,
            ) in tax_details_per_rep_line.items():
    ...
    def _get_invoice_counterpart_amls_for_early_payment_discount_per_payment_term_line(
        ...
        discount_percentage = (
            payment_term_line.move_id.invoice_payment_term_id.discount_percentage
        )
        if not discount_percentage:
            return res
        early_pay_discount_computation = (
            payment_term_line.move_id.invoice_payment_term_id.early_pay_discount_computation
        )
        ...
        if early_pay_discount_computation == "included" and product_lines.tax_ids:
            ...
            for tax_detail in resulting_delta_tax_details.values():
                ...
                grouping_dict = {
                    ...
                    "group_tax_id": (
                        tax_detail["tax_id"] if tax_detail["tax_id"] != tax.id else None
                    ),
                }
            ...
            last_tax_line = (
                list(res["tax_lines"][payment_term_line].values())
                or list(res["base_lines"][payment_term_line].values())
            )[-1]
    ...
    def _cron_account_move_send(self, job_count=10):
        ...
        for _company, moves in all_moves.grouped("company_id").items():
            ...
            moves_by_partner = moves.grouped(
                lambda m: m.send_and_print_values["sp_partner_id"]
            )
            ...
            for partner_id, partner_moves in moves_by_partner.items():
                ...
                partner_moves_error = partner_moves.filtered(
                    lambda m: m.send_and_print_values
                    and m.send_and_print_values.get("error")
                )
                if partner_moves_error:
                    notifications.append(
                        get_account_notification(partner, partner_moves_error, False)
                    )
                partner_moves_success = partner_moves - partner_moves_error
                if partner_moves_success:
                    notifications.append(
                        get_account_notification(partner, partner_moves_success, True)
                    )
    ...
    def _generate_pdf_and_send_invoice(
        ...
        **kwargs,
        ...
        return composer.action_send_and_print(
            ...
            bypass_download=bypass_download,
        )
    ...
    def _get_invoice_legal_documents(self):
        ...
        if self.invoice_pdf_report_id:
            ...
            content, _ = self.env["ir.actions.report"]._render(
    ...
    def get_invoice_pdf_report_attachment(self):
        if len(self) < 2 and self.invoice_pdf_report_id:
            ...
            return (
                ...
                self.message_main_attachment_id.name,
            )
        # all other cases
        ...
        pdf_name = (
            self._get_invoice_report_filename() if len(self) == 1 else "Invoices.pdf"
        )
