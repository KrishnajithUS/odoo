...
class AccountReport(models.Model):
    ...
    def _compute_report_option_filter(self, field_name, default_value=False):
        ...
        for report in self.sorted(lambda x: not x.section_report_ids):
            ...
            elif len(report.section_main_report_ids) == 1 and not self.env[
                "ir.actions.client"
            ].search_count(
                [
    ...
    def _validate_parent_sequence(self):
        ...
        for line in self.line_ids:
            if line.parent_id and line.parent_id not in previous_lines:
                raise ValidationError(
                    _(
                        'Line "%s" defines line "%s" as its parent, but appears before it in the report. '
    ...
    def _check_domain_formula(self):
        for expression in self.filtered(lambda expr: expr.engine == "domain"):
            try:
                ...
                raise UserError(
                    _(
                        "Invalid domain for expression '%s' of line '%s': %s",
                        expression.label,
                        expression.report_line_name,
                        expression.formula,
                    )
                )
                ...
                if not new_tax_tags:
                    ...
                    if former_tax_tags and all(
                        ...
                        if self.pool["account.tax"].name.translate:
                            positive_tags._update_field_translations(
                                "name", {"en_US": f"+{vals['formula']}"}
                            )
                            negative_tags._update_field_translations(
                                "name", {"en_US": f"-{vals['formula']}"}
                            )
                        else:
                ...
                .search(
                    [
                        ...
                        (
                            "formula",
                            "=",
                            tag.with_context(lang="en_US").name[1:],
                        ),  # we escape the +/- sign
