...
class ResCompany(models.Model):
    ...
    account_journal_payment_debit_account_id = fields.Many2one(
        "account.account", string="Journal Outstanding Receipts", check_company=True
    )
    account_journal_payment_credit_account_id = fields.Many2one(
        "account.account", string="Journal Outstanding Payments", check_company=True
    )
    ...
    def _compute_max_tax_lock_date(self):
        for company in self:
            company.max_tax_lock_date = max(
                company.tax_lock_date or date.min,
                company.parent_id.sudo().max_tax_lock_date or date.min,
            )
    ...
    def _get_user_fiscal_lock_date(self):
        ...
        if self.parent_id:
            ...
            lock_date = max(
                lock_date, self.sudo().parent_id._get_user_fiscal_lock_date()
            )
        return lock_date
    ...
    def _get_violated_lock_dates(self, accounting_date, has_tax):
        ...
        if (
            ...
            and tax_lock_date
            and has_tax
            and accounting_date <= tax_lock_date
        ):
    ...
    def create_op_move_if_non_existant(self):
        ...
        if not self.account_opening_move_id:
            self.account_opening_move_id = self.env["account.move"].create(
                self._get_default_opening_move_values()
            )
    ...
    def _with_locked_records(self, records):
        ...
        """
        self._cr.execute(
            f"SELECT * FROM {records._table} WHERE id IN %s FOR UPDATE SKIP LOCKED",
            [tuple(records.ids)],
        )
