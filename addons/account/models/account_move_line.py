...
class AccountMoveLine(models.Model):
    ...
    def get_views(self, views, options=None):
        ...
        if (
            ...
            == "account.move.line.payment.tree"
        ):
    ...
    def _compute_name(self):
        term_by_move = (
            ...
            .sorted(lambda l: l.date_maturity if l.date_maturity else date.max)
            .grouped("move_id")
        )
    ...
    def _compute_epd_needed(self):
        for line in self:
            ...
            epd_needed_vals["amount_currency"] -= line.currency_id.round(
                line.amount_currency * percentage
            )
            epd_needed_vals["balance"] -= line.currency_id.round(
                line.balance * percentage
            )
            epd_needed_vals["price_subtotal"] -= line.currency_id.round(
    ...
    def _check_tax_lock_date(self):
        for line in self.filtered(lambda l: l.move_id.state == "posted"):
            move = line.move_id
            if (
                move.company_id.max_tax_lock_date
    ...
    def init(self):
        ...
        create_index(
            self._cr,
            "account_move_line__unreconciled_index",
            "account_move_line",
    ...
    def _order_field_to_sql(
        ...
        return SQL(
            "%s %s %s", self._field_to_sql(alias, field_name, query), direction, nulls
        )
    ...
    def _reconcile_plan_with_sync(self, plan_list, all_amls):
        ...
        def is_cash_basis_needed(account):
            return account.company_id.tax_exigibility and account.account_type in (
                "asset_receivable",
                "liability_payable",
            )
    ...
    def _convert_to_tax_base_line_dict(self):
        ...
        return self.env["account.tax"]._convert_to_tax_base_line_dict(
            ...
            rate=(
                (abs(self.amount_currency) / abs(self.balance)) if self.balance else 1.0
            ),
        )
