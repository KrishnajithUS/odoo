...
class AccountMoveSend(models.TransientModel):
    ...
    def _get_mail_default_field_value_from_template(
        ...
        if not mail_template:
            return
        return mail_template.with_context(lang=lang)._render_field(
            field, move.ids, **kwargs
        )[move._origin.id]

    def _get_default_mail_lang(self, move, mail_template=None):
        return (
            mail_template._render_lang([move.id]).get(move.id)
            if mail_template
    ...
    def _get_default_mail_attachments_widget(self, move, mail_template):
        return (
            self._get_placeholder_mail_attachments_data(move)
    ...
    def _compute_mode(self):
        for wizard in self:
            wizard.mode = (
                "invoice_single" if len(wizard.move_ids) == 1 else "invoice_multi"
            )
    ...
    def _compute_checkbox_download(self):
        for wizard in self:
            wizard.checkbox_download = (
                wizard.enable_download and wizard.company_id.invoice_is_download
            )
    ...
    def _compute_checkbox_send_mail(self):
        for wizard in self:
            wizard.checkbox_send_mail = (
                wizard.company_id.invoice_is_email and not wizard.send_mail_readonly
            )
    ...
    def _compute_send_mail_extra_fields(self):
        for wizard in self:
            ...
            invoices_without_mail_data = wizard.move_ids.filtered(
                lambda x: not x.partner_id.email
            )
            wizard.send_mail_readonly = invoices_without_mail_data == wizard.move_ids
            if not (
                invoices_without_mail_data
                and wizard.checkbox_send_mail
                or wizard.send_mail_readonly
            ):
                ...
                wizard.send_mail_warning_message = {
                    **(wizard.send_mail_warning_message or {}),
                    "partner_missing_email": {
                        ...
                        "action": partners._get_records_action(
    ...
    def _compute_mail_partner_ids(self):
        for wizard in self:
            if wizard.mode == "invoice_single" and wizard.mail_template_id:
                wizard.mail_partner_ids = wizard._get_default_mail_partner_ids(
                    wizard.move_ids, wizard.mail_template_id, wizard.mail_lang
                )
            else:
    ...
    def action_open_partners_without_email(self, res_ids=None):
        ...
        return (
            self.move_ids.mapped("partner_id")
            .filtered(lambda x: not x.email)
            ._get_records_action(name=_("Partners without email"))
        )
    ...
    def _prepare_invoice_proforma_pdf_report(self, invoice, invoice_data):
        ...
        content, _report_format = (
            self.env["ir.actions.report"]
            .with_company(invoice.company_id)
            ._render("account.account_invoices", invoice.ids, data={"proforma": True})
        )
    ...
    def _link_invoice_documents(self, invoice, invoice_data):
        ...
        invoice.message_main_attachment_id = self.env["ir.attachment"].create(
            invoice_data["pdf_attachment_values"]
        )
    ...
    def _hook_if_errors(self, moves_data, from_cron=False, allow_fallback_pdf=False):
        ...
        for move, move_data in moves_data.items():
            ...
            move.with_context(no_new_invoice=True).message_post(
                body=self._format_error_html(error)
            )
    ...
    def _generate_invoice_documents(self, invoices_data, allow_fallback_pdf=False):
        ...
        for invoice, invoice_data in invoices_data.items():
            if self._need_invoice_document(invoice):
                self._hook_invoice_document_before_pdf_report_render(
        ...
        if allow_fallback_pdf:
            ...
            if invoices_data_pdf_error:
                self._hook_if_errors(
                    invoices_data_pdf_error, allow_fallback_pdf=allow_fallback_pdf
                )
        ...
        for invoice, invoice_data in invoices_data_web_service.items():
            if self._need_invoice_document(invoice) and (
                not invoice_data.get("error") or allow_fallback_pdf
            ):
    ...
    def _download(self, attachment_ids, moves_data=None):
        ...
        if len(attachment_ids) == 1:
            ...
            filename = (
                next(iter(moves_data))._get_invoice_report_filename(extension="zip")
                if len(moves_data) == 1
                else _("invoices") + ".zip"
            )
            return {
    ...
    def _process_send_and_print(
        ...
        moves_data = {
            move: {
                **(
                    move.send_and_print_values
                    if not wizard
                    else wizard._get_wizard_values()
                ),
        ...
        if errors:
            self._hook_if_errors(
                errors, from_cron=from_cron, allow_fallback_pdf=allow_fallback_pdf
            )
        ...
        if success:
            self._hook_if_success(
                success, from_cron=from_cron, allow_fallback_pdf=allow_fallback_pdf
            )
        ...
        to_download = {
            move: move_data
            for move, move_data in moves_data.items()
            if move_data.get("download")
        }
        if to_download:
            attachment_ids = []
            for move, move_data in to_download.items():
                attachment_ids += (
                    self._get_invoice_extra_attachments(move).ids
                    or move_data.get("proforma_pdf_attachment").ids
                )
            if attachment_ids:
    ...
    def action_send_and_print(
        ...
        if (
            ...
            raise UserError(
                _("Please select a mail template to send multiple invoices.")
            )
