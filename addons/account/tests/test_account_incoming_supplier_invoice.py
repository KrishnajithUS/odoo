...
class TestAccountIncomingSupplierInvoice(AccountTestInvoicingCommon):
    ...
    def with_success_decoder(self, omit=None):
        ...
        with patch.object(
            type(self.env["account.move"]), "_get_edi_decoder", get_edi_decoder
        ):
            yield decoded_files
    ...
    def with_simulated_embedded_xml(self, pdf):
        ...
        with patch.object(
            type(self.env["ir.attachment"]), "_decode_edi_pdf", decode_edi_pdf
        ):
            yield xml_filename

    def _assert_extend_with_attachments(
        self, input_values, expected_values=None, new=False, **context
    ):
        if not expected_values:
            expected_values = input_values
        attachments = self.env["ir.attachment"].browse([x.id for x in input_values])
        nb_moves_before = self.env["account.move"].search_count(
        ...
        self.assertEqual(
            current_values, {k.name: v for k, v in expected_values.items()}
        )

        nb_moves_after = self.env["account.move"].search_count(
            [("company_id", "=", self.env.company.id)]
        )
    ...
    def test_extend_with_attachments_multi_pdf(self):
        ...
        with self.with_success_decoder() as decoded_files:
            self._assert_extend_with_attachments(
                {pdf1: 1, pdf2: 2}, new=True, from_alias=True
            )
            self.assertEqual(decoded_files, {pdf1.name, pdf2.name})
        with self.with_success_decoder() as decoded_files:
            self._assert_extend_with_attachments(
                {pdf1: 1, pdf2: 1, gif1: 1, gif2: 1}, new=False
            )
            self.assertEqual(decoded_files, {pdf1.name})
        with self.with_success_decoder() as decoded_files:
            self._assert_extend_with_attachments(
                {pdf1: 1, pdf2: 2, gif1: 3, gif2: 4}, new=True
            )
            self.assertEqual(
                decoded_files, {pdf1.name, pdf2.name, gif1.name, gif2.name}
            )
        with self.with_success_decoder() as decoded_files:
            self._assert_extend_with_attachments(
                {pdf1: 1, pdf2: 2, gif1: 3, gif2: 4},
                expected_values={pdf1: 1, pdf2: 2},
                new=True,
                from_alias=True,
            )
            self.assertEqual(decoded_files, {pdf1.name, pdf2.name})
        ...
        with self.with_success_decoder() as decoded_files:
            self._assert_extend_with_attachments(
                {pdf1: 1, xml1: 1}, new=True, from_alias=True
            )
            self.assertEqual(decoded_files, {xml1.name})
        with self.with_success_decoder() as decoded_files:
        ...
        with self.with_success_decoder() as decoded_files:
            self._assert_extend_with_attachments(
                {xml1: 1, xml2: 2}, new=True, from_alias=True
            )
            self.assertEqual(decoded_files, {xml1.name, xml2.name})
        ...
        with self.with_success_decoder(omit={pdf1.name}) as decoded_files:
            ...
            self.assertEqual(decoded_files, {pdf2.name})
        with self.with_success_decoder() as decoded_files, self.with_simulated_embedded_xml(
            pdf1
        ) as xml_filename:
            self._assert_extend_with_attachments({pdf1: 1, pdf2: 1}, new=False)
