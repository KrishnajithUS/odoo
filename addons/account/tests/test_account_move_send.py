...
class TestAccountComposerPerformance(AccountTestInvoicingCommon, MailCommon):
    ...
    def test_move_composer_single(self):
        ...
        self.assertTrue(
            "is_move_sent" in track_msg.tracking_value_ids.field_id.mapped("name")
        )
        # sent email
        self.assertMailMail(
            test_customer,
...
class TestAccountMoveSendCommon(AccountTestInvoicingCommon):
    ...
    def _get_mail_message(self, move):
        return self.env["mail.message"].search(
            [("model", "=", move._name), ("res_id", "=", move.id)], limit=1
        )
...
class TestAccountMoveSend(TestAccountMoveSendCommon):
    ...
    def test_invoice_multi_with_download(self):
        ...
        self.assertEqual(results["type"], "ir.actions.act_url")
        self.assertEqual(
            (invoice1 + invoice2).mapped("send_and_print_values"), [False, False]
        )
        self.assertTrue(invoice1.invoice_pdf_report_id)
        invoice_attachments = self.env["ir.attachment"].search(
            [
    ...
    def test_with_empty_mail_template_single(self):
        ...
        wizard = self.create_send_and_print(
            invoice, mail_template_id=None, mail_subject=custom_subject
        )

        wizard.action_send_and_print(allow_fallback_pdf=True)
        message = self.env["mail.message"].search(
            [("model", "=", invoice._name), ("res_id", "=", invoice.id)], limit=1
        )
    ...
    def test_with_empty_mail_template_multi(self):
        ...
        custom_subject = "turlututu"
        wizard = self.create_send_and_print(
            (invoice_1 + invoice_2), mail_template_id=None, mail_subject=custom_subject
        )
    ...
    def test_send_and_print_cron(self):
        ...
        self.assertEqual(
            invoices.mapped(lambda inv: bool(inv.send_and_print_values)),
            [True] * len(invoices),
        )
        self.env.ref(
            "account.ir_cron_account_move_send"
        ).method_direct_trigger()  # force processing
    ...
    def test_cron_notifications(self):
        ...
        self.assertTrue(
            all(
                invoice.send_and_print_values
                for invoice in invoices_success + invoices_error
            )
        )
        self.assertEqual(
            invoices_success[0].send_and_print_values.get("sp_partner_id"),
            sp_partner_1.id,
        )
        self.assertEqual(
            invoices_error[0].send_and_print_values.get("sp_partner_id"),
            sp_partner_2.id,
        )
    ...
    def test_send_and_print_only(self):
        invoice = self.init_invoice("out_invoice", amounts=[1000], post=True)
        option_vals = self.env["account.move.send"]._get_wizard_vals_restrict_to(
            {"checkbox_send_mail": True}
        )
        ...
        )  # the download which is a default value didn't happen
