...
class AccountTestInvoicingCommon(TransactionCase):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        if chart_template_ref:
            template_vals = cls.env[
                "account.chart.template"
            ]._get_chart_template_mapping()[chart_template_ref]
            template_module = cls.env.ref(f"base.module_{template_vals['module']}")
            if template_module.state != "installed":
                raise SkipTest(
                    f"Module required for the test is not installed ({template_module.name})"
                )

        cls.company_data_2 = cls.setup_company_data(
            "company_2_data", chart_template=chart_template_ref
        )
    ...
    def setup_company_data(cls, company_name, chart_template=None, **kwargs):
        ...
        return {
            ...
            "default_account_receivable": cls.env["ir.property"]
            ...
            "default_account_assets": cls.env["account.account"].search(
                [("company_id", "=", company.id), ("account_type", "=", "asset_fixed")],
                limit=1,
            ),
    ...
    def _instantiate_basic_test_tax_group(cls, company=None, country=None):
        ...
        vals = {
            "name": "Test tax group",
            "company_id": company.id,
            "tax_receivable_account_id": cls.company_data["default_account_receivable"]
            .sudo()
            .copy({"company_id": company.id})
            .id,
            "tax_payable_account_id": cls.company_data["default_account_payable"]
            .sudo()
            .copy({"company_id": company.id})
            .id,
        }
