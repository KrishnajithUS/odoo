...
class TestAccountTax(AccountTestInvoicingCommon):
    ...
    def test_logging_of_repartition_lines_update_when_tax_is_used(self):
        ...
        previews = self.company_data["default_tax_sale"].message_ids.mapped("preview")
        self.assertIn(
            "Invoice repartition line 3: 100.0 -50.0 (Factor Percent) None ['TaxTag12345'] (Tax Grids)",
            previews,
        )
        self.assertIn(
            "Refund repartition line 3: 100.0 -50.0 (Factor Percent) None 131000 Tax Paid (Account) False True (Use in tax closing)",
            previews,
        )
    ...
    def test_logging_of_repartition_lines_reordering_when_tax_is_used(self):
        ...
        before_last_invoice_rep_line, last_invoice_rep_line = self.company_data[
            "default_tax_sale"
        ].invoice_repartition_line_ids.sorted(key=lambda r: r.sequence)[-2:]

        self.company_data["default_tax_sale"].write(
            {
                "invoice_repartition_line_ids": [
                    Command.update(
                        before_last_invoice_rep_line.id,
                        {
        ...
        previews = self.company_data["default_tax_sale"].message_ids.mapped("preview")
        self.assertIn(
            "Invoice repartition line 2: 251000 Tax Received None (Account) True False (Use in tax closing)",
            previews,
        )
        self.assertIn(
            "Invoice repartition line 3: None 251000 Tax Received (Account) False True (Use in tax closing)",
            previews,
        )
