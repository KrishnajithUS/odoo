...
class TestAccountJournalDashboard(AccountTestInvoicingCommon):
    ...
    def test_purchase_journal_numbers_and_sums(self):
        ...
        self.assertEqual(
            format_amount(self.env, 140, company_currency), dashboard_data["sum_late"]
        )
    ...
    def test_gap_in_sequence_warning(self):
        ...
        self.assertFalse(
            journal._query_has_sequence_holes()
        )  # no gap (with draft moves using sequence numbers), no gap warning
        moves[3].unlink()
        self.assertTrue(
            journal.has_sequence_holes
    ...
    def test_bank_journal_with_default_account_as_outstanding_account_payments(self):
        ...
        bank_journal.inbound_payment_method_line_ids[0].payment_account_id = (
            bank_journal.default_account_id
        )
        payment = self.env["account.payment"].create(
            {
