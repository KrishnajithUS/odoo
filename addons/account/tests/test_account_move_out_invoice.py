...
class TestAccountMoveOutInvoiceOnchanges(AccountTestInvoicingCommon):
    ...
    def test_date_reversal_exchange_move(self):
        ...
        for format_incrementor, expected_date in (
            ...
            with self.subTest(
                ...
                exchange_move = (
                    line_receivable.full_reconcile_id.partial_reconcile_ids.exchange_move_id
                )
    ...
    def test_invoice_mass_posting(self):
        ...
        vam.with_context(
            active_model="account.move", active_ids=[invoice2.id, invoice1.id]
        ).validate_move()
        ...
        invoice.write(
            {
                ...
                "invoice_line_ids": [
                    Command.update(
                        invoice.invoice_line_ids.id, {"account_id": account_b.id}
                    )
                ],
            }
        )
            ...
            invoice.write(
                {
                    "invoice_line_ids": [
                        Command.update(
                            invoice.invoice_line_ids.id, {"account_id": account_a.id}
                        )
                    ]
                }
            )
    ...
    def test_discount_allocation_account_on_invoice(self):
        ...
        self.company_data["company"].account_discount_expense_allocation_id = (
            discount_account
        )
