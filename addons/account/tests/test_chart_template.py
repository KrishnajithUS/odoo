...
class TestChartTemplate(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        with patch.object(
            ...
            cls.env["account.chart.template"].try_loading(
                "test", company=cls.company_1, install_demo=False
            )
            ...
            Command.create(
                {
                    ...
                    "repartition_type": "base",
                    "tag_ids": [
                        Command.set(["account.unsigned_tax_tag", signed_tag.id])
                    ],
                }
            ),
        )
    ...
    def test_update_taxes_creation(self):
        ...
        with patch.object(
            ...
            self.env["account.chart.template"].try_loading(
                "test", company=self.company_1, install_demo=False
            )
    ...
    def test_update_taxes_update(self):
        ...
        updated_tax = self.env["account.tax"].search(
            [("company_id", "=", self.company_1.id), ("name", "=", "Tax 1")]
        )
    ...
    def test_update_taxes_recreation(self):
        ...
        new_tax = self.env["account.tax"].search(
            [("company_id", "=", self.company_1.id), ("name", "=", "Tax 1 modified")]
        )
    ...
    def test_update_taxes_conflict_name(self):
        ...
        tax_1_old = self.env["account.tax"].search(
            [("company_id", "=", self.company_1.id), ("name", "=", "[old] Tax 1")]
        )
        ...
        tax_1_old_second = self.env["account.tax"].search(
            [("company_id", "=", self.company_1.id), ("name", "=", "[old1] Tax 1")]
        )
    ...
    def test_update_account_codes_conflict(self):
        ...
        xmlid_account = self.env.ref(
            f"account.{self.company_1.id}_test_account_income_template"
        )
        self.assertEqual(
            problematic_account,
            xmlid_account,
    ...
    def test_change_coa(self):
        ...
        with (
            ...
            self.env["account.chart.template"].try_loading(
                "other_test", company=self.company_1, install_demo=True
            )
        ...
        with patch.object(
            ...
            self.env["account.chart.template"].try_loading(
                "test", company=self.company_1, install_demo=True
            )
    ...
    def test_parsed_csv_submodel_being_loaded(self):
        ...
        tax_1 = self.env.ref(
            f"account.{self.company_1.id}_tax_1", raise_if_not_found=False
        )
