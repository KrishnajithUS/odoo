...
class TestAccountMoveReconcile(AccountTestInvoicingCommon):
    ...
    def test_reconcile_lines_corner_case_2_zero_amount_currency_same_foreign_currency(
        ...
        self.assertRecordValues(
            full_reconcile.exchange_move_id,
            [{"date": fields.Date.from_string("2017-01-31")}],
        )
        self.assertRecordValues(
            full_reconcile.exchange_move_id.line_ids,
            [
                {
                    "debit": 0.0,
                    "credit": 0.01,
    ...
    def test_reconcile_special_mexican_workflow_1(self):
        ...
        self.assertRecordValues(
            inv2_rec_line.full_reconcile_id, [{"exchange_move_id": None}]
        )
    ...
    def test_reconcile_cash_basis_workflow_single_currency(self):
        ...
        self.assertRecordValues(
            payable_lines_1.full_reconcile_id.exchange_move_id.line_ids,
            [
                {
    ...
    def test_reconcile_cash_basis_exchange_difference_transfer_account_check_entries_4(
        ...
        # Exchange difference
        self.assertFalse(
            receivable_lines.full_reconcile_id.exchange_move_id,
            "No exchange move difference should be created for the full reconcile object ,as there is no cash basis rounding.",
        )
    ...
    def test_matching_loop(self):
        ...
        self.assertEqual(
            len(
                set((debit_a + debit_b + credit_a + credit_b).mapped("matching_number"))
            ),
            1,
        )
        self.assertFalse(
            all(
                ...
                for aml in (debit_a, debit_b, credit_a, credit_b)
            )
        )
    ...
    def test_caba_tax_group(self):
        ...
        exchange_difference_move = invoice.line_ids.filtered(
            lambda line: line.account_id.account_type == "receivable"
    ...
    def test_reconcile_import(self):
        ...
        (line_1 + line_2 + line_3).matching_number = (
            "11111"  # Will be converted to a temporary number
        )
        (line_4 + line_5).matching_number = (
            "22222"  # Will be converted to a temporary number
        )
