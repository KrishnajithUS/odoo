...
class TestMailSchedule(EventCase, MockEmail, CronMixinCase):
    ...
    def test_event_mail_schedule(self):
        ...
        self.assertListEqual(
            registrations.mapped("state"),
            ["open", "open", "draft", "cancel"],
            "Registrations: should be auto-confirmed",
        )
        self.assertListEqual(
            registrations.mapped("create_date"),
            [now] * 4,
        ...
        self.assertEqual(
            ...
            4,
            "event: should have 4 scheduled communication (1 / registration)",
        )
        ...
        self.assertEqual(
            len(self._new_mails),
            4,
            "event: should have 4 scheduled emails (1 / registration)",
        )
        ...
        self.assertEqual(
            len(after_sub_scheduler_2.mail_registration_ids),
            4,
            "event: should have 4 scheduled communication (1 / open registration)",
        )
        self.assertListEqual(
            after_sub_scheduler_2.mail_registration_ids.mapped("mail_sent"),
        ...
        self.assertEqual(
            len(self._new_mails),
            2,
            "event: should have 2 scheduled emails (1 / open registration)",
        )
        self.assertMailMailWEmails(
            [formataddr((reg1.name, reg1.email)), formataddr((reg2.name, reg2.email))],
            "outgoing",
        ...
        with self.mock_datetime_and_now(
            now_start + relativedelta(hours=1)
        ...
        self.assertEqual(
            len(after_sub_scheduler.mail_registration_ids),
            5,
            "event: should have 5 scheduled communication (1 / registration)",
        )
    ...
    def test_event_mail_schedule_on_subscription(self):
        ...
        # event 19
        with self.assertQueryCount(32), self.mock_datetime_and_now(
            reference_now
        ), self.mock_mail_gateway():
        ...
        self.env.invalidate_all()
        # event 50
        with self.assertQueryCount(63), self.mock_datetime_and_now(
            reference_now + relativedelta(minutes=10)
        ), self.mock_mail_gateway():
    ...
    def test_event_mail_schedule_on_subscription_async(self):
        ...
        with self.capture_triggers(cron.id) as capt, self.mock_datetime_and_now(
            reference_now + relativedelta(minutes=10)
        ), self.mock_mail_gateway():
