...
class EventRegistration(models.Model):
    ...
    barcode = fields.Char(
        string="Barcode",
        default=lambda self: self._get_random_barcode(),
        readonly=True,
        copy=False,
    )
    # utm informations
    ...
    def _check_seats_availability(self):
        registrations_confirmed = self.filtered(
            lambda registration: registration.state in ("open", "done")
        )
    ...
    def _compute_phone(self):
        for registration in self:
            if not registration.phone and registration.partner_id:
                ...
                registration.phone = (
                    partner_values.get("phone") or partner_values.get("mobile") or False
                )
    ...
    def get_date_range_str(self, lang_code=False):
        self.ensure_one()
        today_tz = pytz.utc.localize(fields.Datetime.now()).astimezone(
            pytz.timezone(self.event_id.date_tz)
        )
        event_date_tz = pytz.utc.localize(self.event_begin_date).astimezone(
            pytz.timezone(self.event_id.date_tz)
        )
