...
class HrExpenseSheet(models.Model):
    ...
    accounting_date = fields.Date(
        string="Accounting Date", compute="_compute_accounting_date", store=True
    )
    ...
    cannot_approve_reason = fields.Char(
        string="Cannot Approve Reason", compute="_compute_can_approve"
    )
        ...
        for sheet in self:
            if sheet.payment_mode == "company_account":
                if sheet.account_move_ids:
                    ...
                    if sheet.account_move_ids - sheet.account_move_ids.filtered(
                        "reversal_move_id"
                    ):
    ...
    def _compute_main_attachment(self):
        for sheet in self:
            ...
            if (
                not sheet.message_main_attachment_id
                or sheet.message_main_attachment_id not in attachments
            ):
                expenses = sheet.expense_line_ids
                expenses_mma_checksums = expenses.message_main_attachment_id.mapped(
                    "checksum"
                )
                sheet.message_main_attachment_id = (
    ...
    def _compute_can_approve(self):
        is_team_approver = self.user_has_groups(
            "hr_expense.group_hr_expense_team_approver"
        )
        ...
        for sheet in self:
            ...
            if not is_team_approver:
                ...
                current_managers = (
                    sheet_employee.expense_manager_id
                ...
                if sheet_employee.user_id == self.env.user:
                    ...
                    and not is_approver
                    and sheet_employee.expense_manager_id.id != self.env.user.id
                ):
    ...
    def _compute_is_editable(self):
        is_hr_admin = self.user_has_groups(
            "hr_expense.group_hr_expense_manager,base.group_system"
        )
        is_approver = self.user_has_groups("hr_expense.group_hr_expense_user")
        for sheet in self:
    ...
    def _check_payment_mode(self):
        for sheet in self:
            ...
            if expense_lines and any(
                expense.payment_mode != expense_lines[:1].payment_mode
    ...
    def _check_can_create_move(self):
        ...
        missing_email_employees = self.filtered(
            lambda sheet: not sheet.employee_id.work_email
        ).employee_id
        if missing_email_employees:
            action = self.env["ir.actions.actions"]._for_xml_id(
                "hr.open_view_employee_tree"
            )
            action["domain"] = [("id", "in", missing_email_employees.ids)]
            raise RedirectWarning(
                _(
                    "The work email of some employees is missing. Please add it on the employee form"
                ),
                action,
    ...
    def _do_create_moves(self):
        ...
        for move in moves:
            move.message_main_attachment_id = (
                move.attachment_ids[0] if move.attachment_ids else None
            )
        payments = (
            self.env["account.payment"]
            .with_context(**skip_context)
            .create(
                [
    ...
    def _get_expense_account_destination(self):
        ...
        if self.payment_mode == "company_account":
            ...
            if not self.employee_id.sudo().work_contact_id:
                raise UserError(
                    _(
                        "No work contact found for the employee %s, please configure one.",
                        self.employee_id.name,
                    )
                )
            partner = self.employee_id.sudo().work_contact_id.with_company(
                self.company_id
            )
            account_dest = (
