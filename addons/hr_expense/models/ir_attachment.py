...
class IrAttachment(models.Model):
    ...
    def create(self, vals_list):
        ...
        if self.env.context.get("sync_attachment", True):
            expenses_attachments = attachments.filtered(
                lambda att: att.res_model == "hr.expense"
            )
            if expenses_attachments:
                ...
                for expense in expenses.filtered("sheet_id"):
                    ...
                    for attachment in expense.attachment_ids.filtered(
                        lambda att: att.checksum not in checksums
                    ):
                        attachment.copy(
                            {
    ...
    def unlink(self):
        if self.env.context.get("sync_attachment", True):
            ...
            expenses_attachments = self.filtered(
                lambda att: att.res_model == "hr.expense"
            )
            if expenses_attachments:
                expenses = self.env["hr.expense"].browse(
            ...
            sheets_attachments = self.filtered(
                lambda att: att.res_model == "hr.expense.sheet"
            )
            if sheets_attachments:
                ...
                for sheet in sheets.exists():
                    ...
                    attachments_to_unlink += (
                        sheet.expense_line_ids.attachment_ids.filtered(
                            lambda att: att.checksum in checksums
                        )
                    )
