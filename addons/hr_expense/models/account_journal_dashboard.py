...
class AccountJournal(models.Model):
    ...
    def _get_expense_to_pay_query(self):
        return self.env["hr.expense.sheet"]._where_calc(
            self._prepare_expense_sheet_data_domain()
        )
    ...
    def _fill_sale_purchase_dashboard_data(self, dashboard_data):
        ...
        query, params = sale_purchase_journals._get_expense_to_pay_query().select(
            *field_list
        )
        ...
        for journal in sale_purchase_journals:
            ...
            (number_expenses_to_pay, sum_expenses_to_pay) = (
                self._count_results_and_sum_amounts(
