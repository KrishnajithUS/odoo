...
class AccountPayment(models.Model):
    ...
    def _synchronize_to_moves(self, changed_fields):
        # EXTENDS account
        trigger_fields = set(self._get_trigger_fields_to_synchronize()) | {
            "ref",
            "expense_sheet_id",
            "payment_method_line_id",
        }
        if self.expense_sheet_id and any(
            field_name in trigger_fields for field_name in changed_fields
        ):
            raise UserError(
                _(
    ...
    def _must_delete_all_expense_payments(self):
        if (
            self.expense_sheet_id
            and self.expense_sheet_id.account_move_ids.payment_ids - self
        ):  # If not all the payments are to be deleted
            raise UserError(
                _(
                    "You cannot delete only some payments linked to an expense report. All payments must be deleted at the same time."
                )
            )
