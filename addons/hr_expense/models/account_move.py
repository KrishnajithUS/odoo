...
class AccountMove(models.Model):
    ...
    def _creation_message(self):
        if self.expense_sheet_id:
            return _(
                "Expense entry created from: %s", self.expense_sheet_id._get_html_link()
            )
    ...
    def _compute_needed_terms(self):
        ...
        for move in self:
            if (
                ...
                and move.expense_sheet_id.payment_mode == "company_account"
            ):
    ...
    def _reverse_moves(self, default_values_list=None, cancel=False):
        own_expense_moves = self.filtered(
            lambda move: move.expense_sheet_id.payment_mode == "own_account"
        )
    ...
    def _must_delete_all_expense_entries(self):
        if (
            ...
            raise UserError(
                _(
                    "You cannot delete only some entries linked to an expense report. All entries must be deleted at the same time."
                )
            )
