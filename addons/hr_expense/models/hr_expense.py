...
class HrExpense(models.Model):
    ...
    is_editable = fields.Boolean(
        string="Is Editable By Current User", compute="_compute_is_editable"
    )
        ...
        for expense in self:
            if expense.is_multiple_currency:
                if (
                    ...
                    expense.currency_rate = self.env[
                        "res.currency"
    ...
    def _compute_total_amount(self):
        for expense in self:
            if expense.is_multiple_currency:
                ...
                taxes_totals = self.env["account.tax"]._compute_taxes(base_lines)[
                    "totals"
    ...
    def _compute_duplicate_expense_ids(self):
        ...
        expenses = self.filtered(
            lambda expense: expense.employee_id
        ...
        if expenses.ids:
            ...
            for duplicates_ids in (x[0] for x in self.env.cr.fetchall()):
                expenses_duplicates = expenses.filtered(
                    lambda expense: expense.id in duplicates_ids
                )
    ...
    def _convert_to_tax_base_line_dict(
        self, base_line=None, currency=None, price_unit=None, quantity=None
    ):
        self.ensure_one()
        ...
        if "sheet_id" in vals:
            ...
            for expense in self:
                ...
                if vals["sheet_id"] and expense.sheet_id != previous_sheet:
                    for attachment in expense.attachment_ids.with_context(
    ...
    def unlink(self):
        ...
        for sheet in self.sheet_id:
            checksums = set(
                (sheet.expense_line_ids.attachment_ids & self.attachment_ids).mapped(
                    "checksum"
                )
            )
    ...
    def _get_empty_list_mail_alias(self):
        ...
        expense_alias = (
            self.env.ref("hr_expense.mail_alias_expense") if use_mailgateway else False
        )
    ...
    def _get_default_expense_sheet_values(self):
        ...
        for todo in sheets:
            ...
            sheet_name = (
                _("New Expense Report, paid by %(paid_by)s", paid_by=paid_by)
                if len(sheets) > 1
                else False
            )
            if len(todo) == 1:
    ...
    def action_submit_expenses(self):
        if self.filtered(lambda expense: not expense.is_editable):
            raise UserError(_("You are not authorized to edit this expense."))
        sheets = self.env["hr.expense.sheet"].create(
    ...
    def _prepare_payments_vals(self):
        ...
        tax_data = self.env["account.tax"]._compute_taxes(
            [
                self._convert_to_tax_base_line_dict(
                    price_unit=self.total_amount_currency, currency=self.currency_id
                )
            ],
        ...
        for tax_line_data in tax_data["tax_lines_to_add"]:  # Add tax lines
            tax_line_balance = self.company_currency_id.round(
                tax_line_data["tax_amount"] / rate
            )
            ...
            tax_line = {
                ...
                "tax_base_amount": self.company_currency_id.round(
                    tax_line_data["base_amount"] / rate
                ),
                "currency_id": self.currency_id.id,
    ...
    def get_expense_dashboard(self):
        expense_state = {
            ...
            "submitted": {
                ...
                "amount": 0.0,
                "tooltip": _(
                    "Expenses from which the report has been submitted to the approver and is waiting for approval."
                ),
                "currency": self.env.company.currency_id.id,
            },
            "approved": {
                ...
                "amount": 0.0,
                "tooltip": _(
                    "Expenses paid by employee that are approved but not paid yet."
                ),
                "currency": self.env.company.currency_id.id,
            },
        }
        ...
        for state, total_amount_sum in expenses:
            if state in {
                "draft",
                "reported",
            }:  # Fuse the two states into only one "To Submit" state
                state = "to_submit"
