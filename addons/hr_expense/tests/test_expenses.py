...
class TestExpenses(TestExpenseCommon):
    ...
    def test_expense_main_flow(self):
            ...
            [
                {
                    ...
                    "untaxed_amount": 1514.38,
                    "total_tax_amount": 245.62,
                    "state": "draft",
                    "accounting_date": date(2021, 10, 10),
                },
                {
                    "total_amount": 1160.00,
                    "untaxed_amount": 992.65,
                    "total_tax_amount": 167.35,
                    "state": "draft",
        ...
        self.assertRecordValues(
            expense_sheets.expense_line_ids,
            [
                {
        ...
        self.assertRecordValues(
            ...
            [
                ...
                },
                {
                    "state": "done",
                    "payment_state": "paid",
                    "accounting_date": date(2021, 10, 12),
                },  # Set to paid as move is posted directly
            ],
        )
        self.assertRecordValues(
            expense_sheets.expense_line_ids,
            ...
            [
                {
                    ...
                    "invoice_date_due": date(
                        2021, 11, 9
                    ),  # The due date is the one set for the partner
        ...
        self.assertRecordValues(
            expense_sheet_by_company.account_move_ids,
            [
                {
                    "amount_total": 160.00,
                    "ref": "PB 160 + 2*15% 2",
        ...
        company_payment_account_id = self.company_data[
            "company"
        ].account_journal_payment_credit_account_id.id
        # One payment per expense
        self.assertRecordValues(
            expense_sheets.account_move_ids.line_ids,
            [
                # own_account expense sheet move
                {
                    "balance": 123.08,
                    "account_id": product_b_account_id,
                    "name": "expense_employee: PB 160 + 2*15%",
                    "date": date(2021, 10, 10),
                },
                {
                    "balance": 1391.30,
                    "account_id": self.expense_account.id,
                    "name": "expense_employee: PA 2*800 + 15%",
                    "date": date(2021, 10, 10),
                },
                {
                    "balance": 18.46,
                ...
                # company_account expense 2 move
                {
                    "balance": 123.08,
                    "account_id": product_b_account_id,
                    "name": "expense_employee: PB 160 + 2*15% 2",
                    "date": date(2021, 10, 12),
                },
                {
                    "balance": 18.46,
                ...
                # company_account expense 1 move
                {
                    ...
                    "name": "expense_employee: PC 1000 + 15%",
                    "date": date(2021, 10, 11),
                },
                {
                    "balance": 130.43,
        ...
        self.assertRecordValues(
            expense_sheet_by_employee, [{"payment_state": "partial", "state": "done"}]
        )
        ...
        in_payment_state = (
            expense_sheet_by_employee.account_move_ids._get_invoice_in_payment_state()
        )
        self.assertRecordValues(
            expense_sheet_by_employee,
            [{"payment_state": in_payment_state, "state": "done"}],
        )
        self.assertRecordValues(
            expense_sheet_by_employee.expense_line_ids, [{"state": "done"}] * 2
        )
        ...
        self.assertRecordValues(
            expense_sheet_by_employee, [{"payment_state": "paid", "state": "done"}]
        )
        ...
        self.assertRecordValues(
            expense_sheet_by_employee, [{"payment_state": "not_paid", "state": "post"}]
        )
        ...
        self.assertRecordValues(
            expense_sheets.sorted("payment_mode"),
            [
                {
                    "payment_mode": "company_account",
                    "state": "approve",
        ...
        self.assertTrue(
            ...
            == move_twelve_january.payment_id.amount
        )
        self.assertTrue(
            350
            == move_first_january.amount_total
            == move_first_january.payment_id.amount
        )
    ...
    def test_expense_multi_currencies(self):
                    ...
                    Command.create(
                        {
                            ...
                            "tax_ids": [
                                Command.set(
                                    (self.tax_purchase_a.id, self.tax_purchase_b.id)
                                )
                            ],
                        }
                    ),
        ...
        expenses_sheet_currencies_mix = (
            expense_sheet_currency_mix_1
            | expense_sheet_currency_mix_2
            | expense_sheet_currency_mix_3
            | expense_sheet_currency_mix_4
        )
        self.assertRecordValues(
            expenses_sheet_currencies_mix.expense_line_ids,
            [
        ...
        # Back-end override
        expense_sheet_currency_mix_1.expense_line_ids[0].write(
        ...
        self.assertRecordValues(
            expenses_sheet_currencies_mix.expense_line_ids.sorted("id"),
            [
                {
                    "currency_rate": 1.00,
                    "total_amount_currency": 1000.00,
        ...
        self.assertRecordValues(
            expenses_sheet_currencies_mix.account_move_ids,
            [
                {
        ...
        self.assertRecordValues(
            expenses_sheet_currencies_mix.account_move_ids.payment_id,
            [
                {
                    "amount": 1000.00,
        ...
        del expense_values[
            "payment_mode"
        ]  # Sets the default payment_mode (own_account)
        expense_sheet_employee_rounding = self.create_expense_report(
            {"expense_line_ids": [Command.create(expense_values)]}
        )
        expense_sheets_rounding = (
            expense_sheet_company_rounding | expense_sheet_employee_rounding
        )
        self.assertRecordValues(
            expense_sheets_rounding.expense_line_ids,
            [
                {
                    "untaxed_amount_currency": 76.92,
                    "total_amount_currency": 100.00,
                    "total_amount": 14.84,
                    "tax_amount_currency": 23.08,
                    "tax_amount": 3.42,
                },
                {
                    "untaxed_amount_currency": 76.92,
        ...
        self.assertRecordValues(
            expense_sheets_rounding.account_move_ids.line_ids,
            [
                {"balance": 11.42, "amount_currency": 76.92},
                {
                    "balance": 1.71,
                    "amount_currency": 11.54,
                },  # == 3.42 tax_amount & 23.08 tax_amount
                {"balance": 1.71, "amount_currency": 11.54},
                {"balance": -14.84, "amount_currency": -100.00},
                {
                    "balance": 11.42,
                    "amount_currency": 11.42,
                },  # Paid by employee so converted into company_currency
    ...
    def test_attachments_in_move_from_own_expense(self):
        ...
        self.assertRecordValues(
            expense_sheet.account_move_ids.attachment_ids,
            [
                {
                    "raw": b"R0lGODdhAQABAIAAAP///////ywAAAAAAQABAAACAkQBADs=",
                    "name": "file1.png",
    ...
    def test_attachments_in_move_from_company_expense(self):
        ...
        self.assertRecordValues(
            expense_sheet.account_move_ids[0].attachment_ids,
            [
                {
                    "raw": b"R0lGODdhAQABAIAAAP///////ywAAAAAAQABAAACAkQBADs=",
                    "name": "file1.png",
        ...
        self.assertRecordValues(
            expense_sheet.account_move_ids[1].attachment_ids,
            [
                {
                    "raw": b"R0lGODdhAQABAIAAAP///////ywAAAAAAQABAAACAkQBADs=",
                    "name": "file2.png",
    ...
    def test_expense_payment_method(self):
        ...
        self.assertRecordValues(
            expense_sheet, [{"payment_method_line_id": default_payment_method_line.id}]
        )
        ...
        expense_sheet.action_sheet_move_create()
        self.assertRecordValues(
            expense_sheet.account_move_ids.payment_id,
    ...
    def test_create_report_name(self):
        ...
        sheet_names = [
            sheet["name"] for sheet in expenses._get_default_expense_sheet_values()
        ]
    ...
    def test_expense_product_update(self):
        ...
        self.assertRecordValues(
            sheets.expense_line_ids.sorted("name"),
            [
                {
                    "name": "test sheet no update",
                    "price_unit": 100.0,
                    "quantity": 1,
                    "total_amount": 100.0,
                },
                {
                    "name": "test sheet update",
                    "price_unit": 100.0,
                    "quantity": 1,
                    "total_amount": 100.0,
                },
    ...
    def test_expense_mandatory_analytic_plan_product_category(self):
        ...
        with self.assertRaises(
            ...
            expense_sheet.with_context(
                validate_analytic=True
            ).action_approve_expense_sheets()

        expense_sheet.expense_line_ids.analytic_distribution = {
            self.analytic_account_1.id: 100.00
        }
        expense_sheet.with_context(
            validate_analytic=True
        ).action_approve_expense_sheets()
