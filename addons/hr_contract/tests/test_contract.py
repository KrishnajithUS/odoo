...
class TestHrContracts(TestContractCommon):
    ...
    def setUpClass(cls):
        ...
        cls.employee2 = cls.env["hr.employee"].create(
            {
                "name": "Jane Smith",
                "work_permit_expiration_date": date(2015, 11, 1)
    ...
    def test_check_multi_company_contract_expiration(self):
        ...
        self.employee.work_permit_expiration_date = date(2015, 11, 1) + relativedelta(
            days=10
        )

        contract_1 = self.create_contract(
            "open", "normal", date(2015, 11, 1), date(2015, 11, 20), self.employee.id
        )
        contract_2 = self.create_contract(
            "open", "normal", date(2015, 11, 1), date(2015, 11, 13), self.employee2.id
        )
    ...
    def test_running_contract_updates_employee_job_info(self):
        ...
        job = self.env["hr.job"].create({"name": "Software dev"})
    ...
    def test_contract_calendar_update(self):
        ...
        contract1 = self.create_contract(
            "close", "done", date(2024, 1, 1), date(2024, 5, 31)
        )
