...
class ResPartner(models.Model):
    ...
    def _compute_vies_vat_to_check(self):
        ...
        for partner in self:
            ...
            partner.vies_vat_to_check = (
                (
                    country_code.upper() in eu_country_codes
                    or country_code.lower() in _region_specific_vat_codes
                )
                and self._fix_vat_number(country_code + number, partner.country_id.id)
                or ""
            )
    ...
    def _run_vat_test(self, vat_number, default_country, partner_is_company=True):
        ...
        if (
            vat_country_code == "eu"
            and default_country not in self.env.ref("base.europe").country_ids
        ):
    ...
    __check_tin1_ro_natural_persons = re.compile(
