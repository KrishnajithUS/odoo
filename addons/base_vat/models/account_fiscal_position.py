...
class AccountFiscalPosition(models.Model):
    ...
    def adjust_vals_country_id(self, vals):
        ...
        if (
            foreign_vat
            and country_group_id
            and not (self.country_id or vals.get("country_id"))
        ):
            vals["country_id"] = (
                self.env["res.country.group"]
                .browse(country_group_id)
                .country_ids.filtered(lambda c: c.code == foreign_vat[:2].upper())
                .id
                or False
            )
        return vals
    ...
    def _validate_foreign_vat(self):
        for record in self:
            ...
            if record.country_group_id:
                ...
                if not foreign_vat_country:
                    raise ValidationError(
                        _(
                            "The country detected for this foreign VAT number does not match any of the countries composing the country group set on this fiscal position."
                        )
                    )
                if record.country_id:
                    checked_country_code = self.env["res.partner"]._run_vat_test(
                        record.foreign_vat, record.country_id
                    ) or self.env["res.partner"]._run_vat_test(
                        record.foreign_vat, foreign_vat_country
                    )
                ...
                foreign_vat_country = self.env["res.country"].search(
                    [("code", "=", record.foreign_vat[:2].upper())], limit=1
                )
                checked_country_code = self.env["res.partner"]._run_vat_test(
                    record.foreign_vat, foreign_vat_country or record.country_id
                )
                if not checked_country_code:
            ...
            if (
                ...
                and not record.country_group_id
            ):
                raise ValidationError(
                    _(
    ...
    def raise_vat_error_message(self, country=False):
        ...
        error_message = self.env["res.partner"]._build_vat_error_message(
            country_code, self.foreign_vat, fp_label
        )
