    ...
    def _action_reset_password(self):
        ...
        if create_mode:
            ...
            if (
                ...
                and account_created_template._name != "mail.template"
            ):
                _logger.error(
                    "Wrong set password template %r", account_created_template
                )
                return
    ...
    def _alert_new_device(self):
        ...
        if self.email:
            ...
            body = self.env["mail.render.mixin"]._render_template(
                ...
                engine="qweb_view",
                options={"post_process": True},
                add_context=self._prepare_new_device_notice_values(),
            )[self.id]
            mail = (
                self.env["mail.mail"]
                .sudo()
