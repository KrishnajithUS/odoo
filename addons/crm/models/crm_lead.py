...
class Lead(models.Model):
    ...
    )  # inherited via mail.thread.phone
    ...
    def search_fetch(self, domain, field_names, offset=0, limit=None, order=None):
        ...
        search_res = super().search_fetch(
            my_lead_domain, field_names, order=my_lead_order
        )
    ...
    def _get_customer_information(self):
        ...
        for record in self.filtered("email_normalized"):
            ...
            contact_name = (
                ...
                or parse_contact_from_email(record.email_from)[0]
            )
            # Note that we don't attempt to create the parent company even if partner name is set
            values.update(
                record._prepare_customer_values(contact_name, is_company=False)
            )
