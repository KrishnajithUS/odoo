...
from odoo.addons.mail.tests.mail_tracking_duration_mixin_case import (
    MailTrackingDurationMixinCase,
)
...
class TestCRMLead(TestCrmCommon):
    ...
    def test_crm_lead_partner_sync_email_phone(self):
        ...
        with self.debug_mode():
            ...
            partner_mobile_formatted = phone_format(
                partner_mobile, "US", "1", force_format="INTERNATIONAL"
            )
            ...
            new_mobile_formatted = phone_format(
                new_mobile, "US", "1", force_format="INTERNATIONAL"
            )
    ...
    def test_crm_lead_stages(self):
        ...
        with patch.object(self.env.cr, "now", lambda: second_now), freeze_time(
            second_now
        ):
    ...
    def test_crm_lead_update_dates(self):
        ...
        with patch.object(self.env.cr, "now", lambda: updated_time), freeze_time(
            updated_time
        ):
        ...
        with patch.object(self.env.cr, "now", lambda: newer_time), freeze_time(
            newer_time
        ):
        ...
        with patch.object(self.env.cr, "now", lambda: last_time), freeze_time(
            last_time
        ):
        ...
        for team in teams:
            with self.subTest(team=team):
                if team != team_other_comp:
                    self.assertIn(
                        f"<a href='mailto:{team.display_name}'>{team.display_name}</a>",
