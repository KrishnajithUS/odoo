...
class NewLeadNotification(TestCrmCommon):
    ...
    def test_lead_message_get_suggested_recipients_email(self):
        ...
        for lead, expected_suggested in zip(
            ...
            with self.subTest(
                lead=lead, lead_name=lead.name, email_from=lead.email_from
            ):
                    ...
                    if not customer_data:
                        ...
                        for partner_fname in customer_data:
                            found, expected_suggested = (
                                res[index][4][partner_fname],
                                customer_data[partner_fname],
                            )
                            self.assertEqual(
                                found,
                                expected_suggested,
    ...
    def test_lead_message_get_suggested_recipients_values_for_create(self):
        ...
        for partner_name, name, email in [
            ...
            formatted_email = (
                formataddr((name, email)) if name else formataddr((partner_name, email))
            )
            with self.subTest(partner_name=partner_name):
                ...
                self.assertEqual(
                    create_vals, lead1._get_customer_information().get(email, {})
                )
                ...
                self.assertEqual(
                    create_vals["comment"], description
                )  # description -> comment
                # Parent company not created even if partner_name is set
                self.assertFalse(
                    create_vals.get("parent_id")
                )  # not supported, even if partner_name set
                self.assertEqual(
                    create_vals["company_name"], partner_name
                )  # partner_name -> company_name
                expected_company_type = (
                    "company" if partner_name and not name else "person"
                )
                self.assertEqual(
                    create_vals.get("company_type", "person"), expected_company_type
                )
