...
class BaseAutomation(models.Model):
    ...
    trigger = fields.Selection(
        [
            ...
            (
                "on_create",
                "On creation",
            ),  # deprecated, use 'on_create_or_write' instead
    ...
    def _check_trigger_state(self):
        for record in self:
            no_code_actions = record.action_server_ids.filtered(
    ...
    def _compute_trg_date_id(self):
        to_reset = self.filtered(
            lambda a: a.trigger not in TIME_TRIGGERS or len(a.trigger_field_ids) != 1
        )
    ...
    def _compute_trg_date_range_data(self):
        ...
        (self - to_reset).filtered(
            lambda a: not a.trg_date_range_type
    ...
    def _compute_trg_selection_field_id(self):
        to_reset = self.filtered(
            lambda a: a.trigger not in ["on_priority_set", "on_state_set"]
            or len(a.trigger_field_ids) != 1
        )
    ...
    def _compute_trg_field_ref(self):
        to_reset = self.filtered(
            lambda a: a.trigger not in ["on_stage_set", "on_tag_set"]
            or len(a.trigger_field_ids) != 1
        )
    ...
    def _compute_trg_field_ref__model_and_display_names(self):
        ...
        for automation in to_compute:
            ...
            automation.trg_field_ref_model_name = relation
            automation.trg_field_ref_display_name = (
    ...
    def _compute_filter_pre_domain(self):
        to_reset = self.filtered(
            lambda a: a.trigger != "on_tag_set" or len(a.trigger_field_ids) != 1
        )
        ...
        for automation in self - to_reset:
            ...
            automation.filter_pre_domain = (
                f"[('{field}', 'not in', [{value}])]" if value else False
            )

    @api.depends(
        "trigger", "trigger_field_ids", "trg_selection_field_id", "trg_field_ref"
    )
            ...
            if trigger_fields_count == 0:
                ...
                if trigger in ["on_state_set", "on_priority_set"]:
                    ...
                    record.filter_domain = (
                        f"[('{field}', '=', '{value}')]" if value else False
                    )
    ...
    def _compute_on_change_field_ids(self):
        ...
        for record in (self - to_reset).filtered("on_change_field_ids"):
            record.on_change_field_ids = record.on_change_field_ids.filtered(
                lambda field: field.model_id == record.model_id
            )
    ...
    def _compute_trigger_and_trigger_field_ids(self):
        for automation in self:
            ...
            if automation.trigger == "on_stage_set":
                ...
                continue
            if (
                automation.model_id.is_mail_thread
                and automation.trigger in MAIL_TRIGGERS
            ):
                continue

            automation.trigger_field_ids = self.env["ir.model.fields"].search(
                domain, limit=1
            )
            automation.trigger = (
                False if not automation.trigger_field_ids else automation.trigger
            )
    ...
    def _onchange_trigger_or_actions(self):
        ...
        if self.trigger == "on_change" and len(no_code_actions) > 0:
            ...
            action_states = dict(
                self.action_server_ids._fields["state"]._description_selection(self.env)
            )
            return {
                "warning": {
                    "title": _("Warning"),
        ...
        if self.record_getter:
            try:
                record = safe_eval.safe_eval(
                    self.record_getter, self._get_eval_context(payload=payload)
                )
                ...
                if self.log_webhook_calls:
                    ir_logging_sudo.create(
                        self._prepare_loggin_values(
                            message=msg % msg_args, level="ERROR"
                        )
                    )
                raise e
    ...
    def _compute_least_delay_msg(self):
        msg = _(
            "Note that this automation rule can be triggered up to %d minutes after its schedule."
        )
    ...
    def _check_delay(self, automation, record, record_dt):
        if (
            self._get_calendar(automation, record)
            and automation.trg_date_range_type == "day"
        ):
            ...
            delay = DATE_RANGE_FUNCTION[automation.trg_date_range_type](
                automation.trg_date_range
            )
    ...
    def _check(self, automatic=False, use_new_cursor=False):
        ...
        for automation in self.with_context(active_test=True).search(
            [("trigger", "in", TIME_TRIGGERS)]
        ):
            _logger.info("Starting time-based automation rule `%s`.", automation.name)
            last_run = fields.Datetime.from_string(
                automation.last_run
            ) or datetime.datetime.fromtimestamp(0, tz=None)
            ...
            records = (
                self.env[automation.model_name].with_context(context).search(domain)
            )

            def get_record_dt(record):
                # determine when automation should occur for the records
                if (
                    automation.trg_date_id.name == "date_automation_last"
            ...
            for record in records:
                ...
                if (
                    ...
                    is_process_to_run = (
                        ...
                        < past_now[calendar.id]
                    )
                else:
                    is_process_to_run = (
                        last_run
                        <= self._check_delay(automation, record, record_dt)
                        < now
                    )
