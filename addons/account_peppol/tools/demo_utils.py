...
def _mock_deregister_participant(func, self, *args, **kwargs):
    ...
    log_message = _(
        "The peppol status of the documents has been reset when switching from Demo to Live."
    )
    demo_moves._message_log_batch(
        bodies=dict((move.id, log_message) for move in demo_moves)
    )
    ...
    mode_constraint = self.env["ir.config_parameter"].get_param(
        "account_peppol.mode_constraint"
    )
...
def handle_demo(func, self, *args, **kwargs):
    ...
    def get_demo_mode_account_edi_proxy_client_user(self, args, kwargs):
        ...
        demo_param = (
            self.env["ir.config_parameter"].get_param("account_peppol.edi.mode")
            == "demo"
        )
    ...
    demo_mode = (
        get_demo_mode.get(self._name)
        and get_demo_mode[self._name](self, args, kwargs)
        or False
    )
