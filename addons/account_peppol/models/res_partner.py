...
class ResPartner(models.Model):
    ...
    def fields_get(self, allfields=None, attributes=None):
        ...
        existing_selection = res.get("account_peppol_verification_label", {}).get(
            "selection"
        )
        if existing_selection is None:
            return res

        not_valid_format_label = next(
            x
            for x in self._fields["account_peppol_verification_label"].selection
            if x[0] == "not_valid_format"
        )
    ...
    def _get_participant_info(self, edi_identification):
        ...
        peppol_user = self.env.company.sudo().account_edi_proxy_client_ids.filtered(
            lambda user: user.proxy_type == "peppol"
        )
        edi_mode = (
            peppol_user
            and peppol_user.edi_mode
            or self.env["ir.config_parameter"]
            .sudo()
            .get_param("account_peppol.edi.mode")
        )
    ...
    def _check_document_type_support(self, participant_info, ubl_cii_format):
        ...
        document_type = self.env["account.edi.xml.ubl_21"]._get_customization_ids()[
            ubl_cii_format
        ]
    ...
    def button_account_peppol_check_partner_endpoint(self):
        ...
        if (
            ...
            self.account_peppol_is_endpoint_valid = bool(
                self._check_peppol_participant_exists(
                    edi_identification, ubl_cii_format=self.ubl_cii_format
                )
            )
        return False
