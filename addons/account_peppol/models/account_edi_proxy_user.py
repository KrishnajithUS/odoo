                ...
                for uuid, content in all_messages.items():
                    ...
                    try:
                        ...
                        if partner_endpoint:
                            move._message_log(
                                body=_(
                                    "Peppol document has been received successfully. Sender endpoint: %s",
                                    partner_endpoint,
                                )
                            )
                        else:
            ...
            for uuids in split_every(BATCH_SIZE, message_uuids.keys()):
                ...
                for uuid, content in messages_to_process.items():
                    ...
                    if content.get("error"):
                        ...
                        move._message_log(
                            body=_("Peppol error: %s", content["error"]["message"])
                        )
                        continue

                    move.peppol_move_state = content["state"]
                    move._message_log(
                        body=_("Peppol status update: %s", content["state"])
                    )
