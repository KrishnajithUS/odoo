...
class AccountMoveSend(models.TransientModel):
    ...
    def _compute_checkbox_send_peppol(self):
        for wizard in self:
            wizard.checkbox_send_peppol = (
                wizard.enable_peppol and not wizard.peppol_warning
            )
        ...
        for wizard in self:
            ...
            wizard.enable_peppol = (
                ...
                and (
                    ...
                    or any(
                        m.ubl_cii_xml_id
                        and m.peppol_move_state not in ("processing", "done")
    ...
    def action_send_and_print(
        ...
        if all(
            [
                self.checkbox_send_peppol,
                self.enable_peppol,
                self.enable_ubl_cii_xml,
                not self.checkbox_ubl_cii_xml,
            ]
        ):
            ...
            if invoice_data.get("send_peppol"):
                ...
                if not partner.peppol_eas or not partner.peppol_endpoint:
                    ...
                    invoice_data["error"] = _(
                        "The partner is missing Peppol EAS and/or Endpoint identifier."
                    )
                    continue
        ...
        try:
            ...
            if response.get("error"):
                ...
                invoices._message_log_batch(
                    bodies=dict((invoice.id, log_message) for invoice in invoices)
                )
