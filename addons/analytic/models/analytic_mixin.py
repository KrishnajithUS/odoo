...
class AnalyticMixin(models.AbstractModel):
    ...
    def _search_analytic_distribution(self, operator, value):
        if operator == "in" and isinstance(value, (tuple, list)):
        ...
        elif operator in ("=", "!=", "ilike", "not ilike") and isinstance(
            value, (str, bool)
        ):
            ...
            operator_inselect = (
                "inselect" if operator in ("=", "ilike") else "not inselect"
            )
        else:
    ...
    def filtered_domain(self, domain):
        ...
        return super(
            AnalyticMixin, self.with_context(distribution_ids=True)
        ...
        if self.env.context.get("validate_analytic", False):
            ...
            for analytic_account_ids, percentage in (
                ...
                for analytic_account in (
                    ...
                    .browse(map(int, analytic_account_ids.split(",")))
                    .exists()
                ):
    ...
    def _apply_analytic_distribution_domain(self, domain):
        return [
            (
                ...
                if len(leaf) == 3
                and leaf[0] == "analytic_distribution"
                and isinstance(leaf[2], (str, tuple, list))
                else leaf
            )
            for leaf in domain
        ]
    ...
    def _get_analytic_account_ids(self) -> list[int]:
        ...
        return [
            int(account_id)
            for ids in (self.analytic_distribution or {})
