...
class HrEmployeeBase(models.AbstractModel):
    ...
    name_work_location_display = fields.Char(
        compute="_compute_name_work_location_display"
    )
    ...
    def get_views(self, views, options=None):
        ...
        if "list" in res["views"]:
            res["views"]["list"]["arch"] = res["views"]["list"]["arch"].replace(
                "name_work_location_display", dayfield
            )
        return res
    ...
    def _compute_name_work_location_display(self):
        ...
        for employee in self:
            ...
            employee.name_work_location_display = (
                current_location_id.name if current_location_id else unspecified
            )
    ...
    def _compute_presence_icon(self):
        ...
        for employee in self:
            today_employee_location_id = (
                employee.exceptional_location_id or employee[dayfield]
            )
            if not today_employee_location_id or employee.hr_icon_display.startswith(
                "presence_holiday"
            ):
                continue
