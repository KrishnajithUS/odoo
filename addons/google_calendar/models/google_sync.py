...
class GoogleSync(models.AbstractModel):
    ...
    def _handle_allday_recurrences_edge_case(self, records, vals_list):
        ...
        if vals_list and self._name == "calendar.event":
            ...
            records_to_skip = records.filtered(
                lambda r: r.need_sync
                and r.allday
                and r.recurrency
                and not r.recurrence_id
            )
    ...
    def _sync_google2odoo(self, google_events: GoogleEvent, default_reminders=()):
        ...
        for gevent in pending:
            ...
            if not odoo_record_write_date or updated >= pytz.utc.localize(
                odoo_record_write_date
            ):
                vals = dict(
            ...
            if token:
                try:
                    ...
                    google_id = google_service.insert(
                        ...
                        token=token,
                        timeout=timeout,
                        callback_method=self.write_insertion_values,
                    )
    ...
    def _get_sync_partner(self, emails):
        ...
        unsorted_partners = self.env["res.partner"].browse(
            [p.id for p in partners if p.id]
        )
