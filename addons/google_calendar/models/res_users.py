...
class User(models.Model):
    ...
    def _sync_google_calendar(self, calendar_service: GoogleCalendarService):
        ...
        synced_recurrences = (
            ...
            .with_context(write_dates=recurrences_write_dates)
            ._sync_google2odoo(recurrences)
        )
        synced_events = (
            self.env["calendar.event"]
            .with_context(write_dates=events_write_dates)
            ._sync_google2odoo(
                ...
                if not event_id:
                    events, next_sync_token, default_reminders = (
                        calendar_service.get_events(
                            self.google_calendar_account_id.calendar_sync_token,
                            token=token,
                        )
                    )
                else:
